...
<h3 class="page-title">
	<small><?php echo fc_lang('运用自定义字段功能会大大增强FineCMS的内容管理功能'); ?></small>
</h3>
...
<div class="portlet light bordered">
	<div class="portlet-body">
		...
		<tr style="color:<?php echo $group[$t['fieldname']]; ?>;" <?php if ($t['fieldtype']!='Group' && $t['fieldtype']!='Merge' && $group[$t['fieldname']]) { ?> class="iii"<?php } ?>>
            ...
            <?php if ($t['issystem']) { ?>
            <a href="javascript:;" class="adel"><?php echo fc_lang('不可删除'); ?></a>
            <?php } else { ?>
		...
		<tr class="mtable_bottom">
			<th align="right"><input name="dr_select" type="checkbox" class="toggle md-check" onClick="dr_selected()" /></th>
			<td colspan="11" align="left">
            <label><button type="button" class="btn red btn-sm" name="button" onClick="$('#action').val('del');return dr_confirm_set_all('<?php echo fc_lang('您确定要这样操作吗？'); ?>')"> <i class="fa fa-trash"></i> <?php echo fc_lang('删除'); ?> </button></label>
