...
function dr_copy_module(url) {
	var throughBox = $.dialog.through;
	var dr_Dialog = throughBox({title: "<?php echo fc_lang('复制模块'); ?>"});
	$.ajax({type: "GET", url:url, dataType:'text', success: function (text) {
			var win = $.dialog.top;
			dr_Dialog.content(text);
			dr_Dialog.button({name: "<?php echo fc_lang('复制'); ?>", callback:function() {
		...
		<div class="portlet-body">
			<div class="table-scrollable v3table">
				<table class="table">
					<thead>
					<tr>
						<th width="40" align="center"><?php echo fc_lang('排序'); ?></th>
						<th width="40" align="center"><?php echo fc_lang('可用'); ?></th>
						<th width="40" align="center"><?php echo fc_lang('类型'); ?></th>
								...
								<div class="btn-group">
									...
									<ul class="dropdown-menu" role="menu">
										<li role="presentation">
											<a role="menuitem"href="javascript:;"> <?php echo fc_lang('主表: %s', SITE_ID.'_share'); ?></a>
										</li>
										<li role="presentation">
											<a role="menuitem"href="javascript:;"> <?php echo fc_lang('从表: %s', SITE_ID.'_'.$dir); ?></a>
										</li>
										<li role="presentation">
											<a role="menuitem"href="javascript:;"> <?php echo fc_lang('附表: %s', SITE_ID.'_'.$dir.'_data_0 ...'); ?></a>
										</li>
										<li role="presentation">
											<a role="menuitem"href="javascript:;"> <?php echo fc_lang('栏目: %s', SITE_ID.'_category'); ?></a>
										</li>
									</ul>
								</div>
								<?php } else { ?>
					...
					<tr class="dr_none">
						...
						<td align="center">
							<?php if ($this->ci->is_auth('module/install')) {  if ($t['space'] || $t['nodb']) { ?>
								...
								<?php } else { ?>
									<a href="javascript:void(0);" onclick="dr_install_share('内容模块分为独立模块和共享模块 <br> 独立模块: 有各自的主表和栏目表,相互无关联,完全独立化管理,负载能力极高 <br> 共享模块: 主表和栏目表共享,方便栏目管理', '<?php echo dr_url('module/install_all', array('dir'=>$dir)); ?>')" style="color:#00F">
								<?php }  } else { ?>
