<table class="table dr_table">
    ...
    <tr>
        ...
        <th class="<?php echo ns_sorting('title'); ?>" name="title"><?php echo fc_lang('名称'); ?></th>
        <th class="<?php echo ns_sorting('catid'); ?>" name="catid"><?php echo fc_lang('栏目分类'); ?></th>
    ...
    <tr class="dr_row_hide" id="dr_row_<?php echo $t['id']; ?>">
        ...
        <td><a title="<?php echo dr_clearhtml($t['title']); ?>" class="onloading" href="<?php echo dr_url(APP_DIR.'/home/edit',array('id'=>$t['id'])); ?>"><?php if ($t['thumb']) { ?><img src="<?php echo THEME_PATH; ?>admin/images/img.png" align="absmiddle" height="18" width="15">&nbsp;<?php }  echo dr_keyword_highlight(dr_strcut(dr_clearhtml($t['title']), 40), $param['keyword']);  if ($t['link_id'] >0) { ?><img align="absmiddle" src="<?php echo THEME_PATH; ?>admin/images/link2.png"><?php } ?></a></td>
        <td><a title="<?php echo dr_cat_value($t['catid'], 'name'); ?>" href="<?php if ($flag) {  echo dr_url(APP_DIR.'/home/index', array('flag'=>$flag,'catid'=>$t['catid']));  } else {  echo dr_url(APP_DIR.'/home/index', array('catid'=>$t['catid']));  } ?>"><?php echo dr_strcut(dr_cat_value($t['catid'], 'name'), 12); ?></a></td>
        ...
        <td ><?php if ($t['status'] == 9) { ?><a class="btn blue btn-xs" href="javascript:dr_status('<?php echo $t['id']; ?>', '<?php echo $t['status']; ?>');"><?php echo fc_lang('正常'); ?></a><?php } else { ?><a class="btn red btn-xs" href="javascript:dr_status(<?php echo $t['id']; ?>, '<?php echo $t['status']; ?>');"><?php echo fc_lang('关闭'); ?></a><?php } ?></td>
        <td>
            <label>
            <a href="<?php if ($t['status'] == 9) {  echo $t['url'];  } else {  echo APP_DIR; ?>/index.php?c=show&id=<?php echo $t['id'];  } ?>" target="_blank" class="btn btn-xs yellow"> <i class="fa fa-send"></i> <?php echo fc_lang('访问'); ?> </a>
            </label>
            <?php if ($form) { ?>
            <label>
            <div class="btn-group">
            ...
            <label>
            <a class="btn btn-xs blue onloading" href="<?php echo dr_url(APP_DIR.'/comment/index',array('cid'=>$t['id'])); ?>"> <i class="fa fa-comments"></i> <?php echo fc_lang('评论管理'); ?></a>
            </label>
            <?php } ?>
        </td>
    </tr>
