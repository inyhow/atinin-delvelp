			...
			$.ajax({type: "POST",dataType:"json", url: "<?php echo dr_url(APP_DIR.'/home/status'); ?>&id="+id+"&v="+v, success: function(data) {
				...
				if (data.status == 1) {
					...
					if (data.id) {
						...
						$.post("<?php echo $html_url; ?>c=home&m=create_list_html&id="+data.id, {}, function(){});
					}
					setTimeout('window.location.reload(true)', 3000); // 刷新页
				} else {
					dr_tips(data.code);
...
<div class="page-bar">
	<ul class="page-breadcrumb mylink">
		<?php echo $menu['link']; ?>
		<li><a href="<?php echo $ci->duri->uri2url($post); ?>"> <i class="fa fa-plus"></i> <?php echo fc_lang('发布'); ?></a></li>
	</ul>
