...
class Account extends M_Controller {
        ...
        if (IS_POST) {
            ...
            if (!isset($data['error']) && $this->member['groupid'] == 2) {
                ...
                if (isset($data['error'])) {
                    ...
                    $this->member_msg(fc_lang('保存成功'), '/index.php?s=member', 1,1);
                }
            } else {
                ...
                if (isset($data['error'])) {
                    ...
                    if ($result) {
                        ...
                        if (!$this->db
                            ...
                            $this->member_model->update_score(0, $this->uid, (int)$this->member_rule['complete_experience'], 'complete', fc_lang('完善资料'));
                        }
                        // 完善资料金币处理
                        if (!$this->db
                                  ->where('uid', $this->uid)
                                  ...
                                  ->where('mark', 'complete')
                                  ->count_all_results('member_scorelog')) {
                            $this->member_model->update_score(1, $this->uid, (int)$this->member_rule['complete_score'], 'complete', fc_lang('完善资料'));
                        }
                    }
                    //$this->member_msg(fc_lang('操作成功，正在刷新...'), dr_member_url('account/index'), 1);
                    $this->member_msg(fc_lang('保存成功'), '/index.php?s=member', 1,1);
    ...
    public function sendsms() {
        ...
        // 是否已经认证过
        if ($this->member['ismobile'] && $this->member['phone']) {
            exit(dr_json(0, fc_lang('您已经认证过了')));
        }
    ...
    public function jie() {
        ...
        if (!$this->db
                ...
                ->where('mark', 'jie_'.$id)
                ->count_all_results('member_scorelog')) {
            $this->member_model->update_score(0, $this->uid, (int)$this->member_rule['jie_experience'], 'jie_'.$id, 'OAuth账号解绑');
        }
        ...
        if (!$this->db
                ...
                ->where('mark', 'jie_'.id)
                ->count_all_results('member_scorelog')) {
            $this->member_model->update_score(1, $this->uid, (int)$this->member_rule['jie_score'], 'jie_'.$id, 'OAuth账号解绑');
        }
    ...
    public function bang() {
        ...
        $config	= $oauth[$appid];
        if (!$config) {
            $this->msg(fc_lang('OAuth错误: 缺少OAuth参数'));
        }
                ...
                if (is_array($user) && $user['oid']) {
                    if ($uid = $this->member_model->OAuth_bang($appid, $user)) {
                        ...
                        if (!$this->db
                                  ...
                                  ->count_all_results('member_scorelog')) {
                            $this->member_model->update_score(0, $this->uid, (int)$this->member_rule['bang_experience'], 'bang_'.$appid, 'OAuth账号绑定');
                        }
                        ...
                        if (!$this->db
                                  ...
                                  ->where('mark', 'bang_'.$appid)
                                  ->count_all_results('member_scorelog')) {
                            $this->member_model->update_score(1, $this->uid, (int)$this->member_rule['bang_score'], 'bang_'.$appid, 'OAuth账号绑定');
                        }
    ...
    public function upload() {
        ...
        if (!$post) {
            exit(function_exists('iconv') ? iconv('UTF-8', 'GBK', '环境php://input不支持') : 'php://input NULL');
        }
        ...
        if (!$this->db
                  ...
                  ->where('mark', 'avatar')
                  ->count_all_results('member_scorelog')) {
            $this->member_model->update_score(0, $this->uid, (int)$this->member_rule['avatar_experience'], 'avatar', fc_lang('上传头像'));
        }
        ...
        if (!$this->db
                  ...
                  ->where('mark', 'avatar')
                  ->count_all_results('member_scorelog')) {
            $this->member_model->update_score(1, $this->uid, (int)$this->member_rule['avatar_score'], 'avatar', fc_lang('上传头像'));
        }
    ...
    public function upgrade() {
        ...
        if ($id) {
            ...
            if ($id == $this->member['groupid']) {
                ...
                $renew = TRUE;
                if ($time > 2000000000) {
                    $this->member_msg(fc_lang('当前会员组有效期为永久，因此无法重复升级'));
                }
            } else {
                // 表示申请其他组
                $time = 0;
                $renew = FALSE;
                if (!$this->get_cache('member', 'group', $this->member['groupid'], 'allowapply_orther')) {
                    $this->member_msg(fc_lang('该会员组不允许申请其他组'));
                }
            }
            ...
            if ($group['unit'] == 1) {
                ...
                if ($this->member['score'] - $value < 0) {
                    $this->member_msg(fc_lang(SITE_SCORE.'不足！本次需要%s'.SITE_SCORE.'，当前余额%s'.SITE_SCORE.'', $value, $this->member['score']));
                }
                $this->member_model->update_score(1, $this->uid, -$value, '', '会员组升级消费：%s'.$group['name']);
            } else {
                // 人民币扣减
                if ($this->member['money'] - $group['price'] < 0) {
                    $this->member_msg(fc_lang(SITE_MONEY.'不足！本次需要%s'.SITE_MONEY.'，当前余额%s'.SITE_MONEY.'', $group['price'], $this->member['money']));
                }
