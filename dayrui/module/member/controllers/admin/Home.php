...
class Home extends M_Controller {
    ...
    public function index() {
        ...
        if (IS_POST && $this->input->post('action')) {
            ...
            switch ($action) {
                ...
                case 'order':
                    // 排序
                    if (!$this->is_auth('member/admin/home/order')) {
                        exit(dr_json(0, fc_lang('您无权限操作')));
                    }

                    $_data = $this->input->post('data');
                    foreach ($ids as $id) {
                        $this->db->where_in('uid', $id)->update('member', array('displayorder' => $_data[$id]['displayorder']));
                    }
                    $this->system_log('会员排序【#'.@implode(',', $ids).'】'); // 记录日志
                    exit(dr_json(1, fc_lang('操作成功，正在刷新...')));
        ...
        // 数据库中分页查询
        list($data, $param) = $this->member_model->limit_page($param, max($_GET['page'], 1), $_GET['order'], $_GET['total']);
        
        //等级
        $p_level = $this->db->where('groupid','3')->get('member_level')->result_array();
        $c_level = $this->db->where('groupid','4')->get('member_level')->result_array();
    ...
    public function add() {
        ...
        if (IS_POST) {
            ...
            if (!$data['groupid']) {
                exit(dr_json(0, fc_lang('先选择一个会员组吧'), 'groupid'));
            }
            
            if ($all) {
                // 批量添加
                if (!$info) {
                    exit(dr_json(0, fc_lang('批量注册信息填写不完整'), 'info'));
                }
            ...
            if (!$edit['groupid']) {
                ...
                if ($edit['email'] != $data['email']) {
                    if (!preg_match('/^[\w\-\.]+@[\w\-\.]+(\.\w+)+$/', $edit['email'])) {
                        $this->admin_msg(fc_lang('邮箱格式不正确'));
                    }
                    if ($this->db->where('email', $edit['email'])->where('uid<>', $uid)->count_all_results('member')) {
                        $this->admin_msg(fc_lang('该邮箱【%s】已经被注册', $edit['email']));
                    }
                    if ($is_uc) {
    ...
    public function add_experience() {
        ...
        if (IS_POST) {
            ...
            if (!$value) {
                exit(dr_json(0, fc_lang('请填写变动数量值'), 'value'));
            }
