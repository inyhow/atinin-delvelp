...
class Menu extends M_Controller {
		...
		if ($data) {
			foreach($data as $t) {
				...
				if ($this->is_auth('member/admin/menu/del')) {
					$t['option'].= '<a class="adel" title="'.fc_lang('删除').'" href="javascript:;" onClick="return dr_dialog_del(\''.fc_lang('您确定要这样操作吗？').'\',\''.dr_url('member/menu/del',array('id' => $t['id'])).'\');"> <i class="fa fa-trash"></i> '.fc_lang('删除').'</a>';
				}
				$tree[$t['id']] = $t;
			}
		}
    ...
    public function edit() {
		...
		$data = $this->db
					 ...
					 ->get('member_menu')
					 ->row_array();
		if (!$data) exit(fc_lang('对不起，数据被删除或者查询不存在'));
