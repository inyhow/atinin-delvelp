...
class Group extends M_Controller {
    ...
    public function add() {
		...
		$template = array_diff(dr_dir_map(FCPATH.'module/member/templates/member/', 1), array('admin'));
		$template2 = dr_dir_map(FCPATH.'dayrui/templates/member/', 1);
		...
		$this->template->assign(array(
			...
			'overdue' => $overdue,
			'is_theme' => 0,
			'mtemplate' => $template ? array_merge($template, $template2) : $template2,
		));
