...
class Api extends M_Controller {
    ...
    public function ologin() {
        ...
        // 未登录的情况下
        if (!$this->member) {
            $this->admin_msg(fc_lang('务必要在会员中心登录一次,才能进行授权登录'), MEMBER_URL);
        }

        // 非管理员无权操作
        if ($this->member['adminid'] != 1) {
            $this->admin_msg($this->member['username'].'：'.fc_lang('您无权限操作'));
        }
    ...
    public function related() {
        ...
        // 登陆判断
        if (!$this->uid) {
            $this->admin_msg(fc_lang('会话超时，请重新登录'));
        }

        // 参数判断
        $dirname = $this->input->get('module');
        if (!$dirname) {
            $this->admin_msg(fc_lang('模块module参数不存在'));
        }
        ...
        if (!$module) {
            $this->admin_msg(fc_lang('此模块（%s）缓存不存在', $dirname));
        }
    ...
    public function linkage() {
        ...
        foreach ($linkage as $v) {
            if ($v['pid'] == $pid) {
                $json[] = array('region_id' => $v['ii'], 'region_name' => $v['name']);
            }
        }
    ...
    public function sns_upload() {
        ...
        if (!$uid) {
            exit(json_encode(array('status' => 0, 'data' => fc_lang('抱歉！游客不允许上传附件'))));
        }
        ...
        if (!$this->member['adminid'] && !$member_rule['is_upload']) {
            exit(json_encode(array('status' => 0, 'data' => fc_lang('抱歉！您的会员组无权上传附件'))));
        }
        ...
        if (!$this->member['adminid'] && $member_rule['attachsize']) {
            ...
            if ($filesize > $member_rule['attachsize'] * 1024 * 1024) {
                exit(json_encode(array('status' => 0, 'data' => fc_lang('附件空间不足！您的附件总空间%s，现有附件%s。', $member_rule['attachsize'].'MB', dr_format_file_size($filesize)))));
            }
        }
    ...
    public function ajax_upload() {
        ...
        // 游客不允许上传
        if (!$this->member) {
            echo json_encode(array('code'=>0, 'msg'=>fc_lang('抱歉！游客不允许上传附件11'), 'url'=> '', 'id'=>''));exit;
        }
        ...
        // 是否允许上传附件
        if (!$this->member['adminid'] && !$member_rule['is_upload']) {
            echo json_encode(array('code'=>0, 'msg'=>fc_lang('抱歉！您的会员组无权上传附件'), 'url'=> '', 'id'=>''));exit;
        }
        ...
        if (!$this->member['adminid'] && $member_rule['attachsize']) {
            ...
            $filesize = (int)$data['filesize'];
            if ($filesize > $member_rule['attachsize'] * 1024 * 1024) {
                echo json_encode(array('code'=>0, 'msg'=>fc_lang('附件空间不足！您的附件总空间%s，现有附件%s。', $member_rule['attachsize'].'MB', dr_format_file_size($filesize)), 'url'=> '', 'id'=>''));exit;
            }
        }
    ...
    public function down_file() {
        ...
        if (!$this->uid) {
            exit(dr_json(0, fc_lang('抱歉！游客不允许上传附件')));
        }
        ...
        // 是否允许上传附件
        if (!$this->member['adminid'] && !$member_rule['is_upload']) {
            exit(dr_json(0, fc_lang('抱歉！您的会员组无权上传附件')));
        }
        ...
        if (!$this->member['adminid'] && $member_rule['attachsize']) {
            ...
            $filesize = (int)$data['filesize'];
            if ($filesize > $member_rule['attachsize'] * 1024 * 1024) {
                exit(dr_json(0, fc_lang('附件空间不足！您的附件总空间%s，现有附件%s。', $member_rule['attachsize'].'MB', dr_format_file_size($filesize))));
            }
        }
    ...
    public function swfupload() {
        ...
        if (!$this->member['adminid'] && $member_rule['attachsize']) {
            ...
            $filesize = (int)$data['filesize'];
            if ($filesize > $member_rule['attachsize'] * 1024 * 1024) {
                exit('0,'.fc_lang('附件空间不足！您的附件总空间%s，现有附件%s。', $member_rule['attachsize'].'MB', dr_format_file_size($filesize)));
            }
        }
    ...
    public function new_ajax_upload() {
        ...
        // 游客不允许上传，未获取到会员信息时提示游客无法上传
        if (!$this->member) {
            echo json_encode(array('code'=>0, 'msg'=>fc_lang('抱歉！游客不允许上传附件')));exit;
        }
        ...
        // 是否允许上传附件
        if (!$this->member['adminid'] && !$member_rule['is_upload']) {
            echo json_encode(array('code'=>0, 'msg'=>fc_lang('抱歉！您的会员组无权上传附件')));exit;
        }
        ...
        if (!$this->member['adminid'] && $member_rule['attachsize']) {
            ...
            $filesize = (int)$data['filesize'];
            if ($filesize > $member_rule['attachsize'] * 1024 * 1024) {
                echo json_encode(array('code'=>0, 'msg'=>fc_lang('附件空间不足！您的附件总空间%s，现有附件%s。', $member_rule['attachsize'].'MB', dr_format_file_size($filesize))));exit;
            }
        }
    ...
    public function ueupload() {
        ...
        if (!$this->uid) {
            exit("{'url':'','title':'','original':'','state':'".fc_lang('会话超时，请重新登录')."'}");
        }

        // 是否允许上传附件
        if (!$this->member['adminid'] && !$this->member_rule['is_upload']) {
            exit("{'url':'','title':'','original':'','state':'".fc_lang('抱歉！您的会员组无权上传附件')."'}");
        }
        ...
        if (!$this->member['adminid'] && $this->member_rule['attachsize']) { // 附件总大小判断
            ...
            if ($filesize > $this->member_rule['attachsize'] * 1024 * 1024) {
                exit("{'url':'','title':'','original':'','state':'".fc_lang('附件空间不足！您的附件总空间%s，现有附件%s。', $this->member_rule['attachsize'].'MB', dr_format_file_size($filesize))."'}");
            }
        }
        $path = SYS_UPLOAD_PATH.'/'.date('Ym', SYS_TIME).'/';
        if (!is_dir($path)) {
    ...
    public function checktitle() {
        ...
        $num = $this->site[SITE_ID]->where('id<>', $id)->where('title', $title)->count_all_results(SITE_ID.'_'.$module);
        if ($num) {
            exit(fc_lang('<font color=red>重复</font>'));
        } else {
            exit('');
        }
    }
    ...
    public function file() {
        ...
        if (!$info) {
            $this->admin_msg(fc_lang('附件不存在或者已经被删除'));
        }
        ...
        if ($this->member_rule['download_score']
            ...
            if ($this->member_rule['download_score'] + $this->member['score'] < 0) {
                $this->admin_msg(fc_lang('下载附件需要%s%s', SITE_SCORE, abs($this->member_rule['download_score'])));
            }
            // 金币扣减
