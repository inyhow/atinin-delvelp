...
class Member_model extends CI_Model {
    ...
    public function get_member($uid) {
        ...
        // finewms对接
        $data['weixin'] = defined('BRANCH_WEIXIN') ? $this->db->where('uid', $uid)->get('weixin_user')->row_array() : array();
    ...
    public function login($username, $password, $expire, $back = 0, $is_uid = 0) {
        ...
        if ($MEMBER['setting']['ucenter']) {
            ...
            if ($uid <= 0) {
                ...
                if ($data) {
                    list($uid, $username, $password, $email) = uc_user_login($data['username'], $password);
                }
            }
            if ($uid > 0) {
                // 当前会员不存在时就重新注册
        ...
        foreach ($MEMBER['synurl'] as $url) {
            ...
            //$synlogin = file_get_contents($url.'/index.php?c=api&m=synlogin&expire='.$expire.'&code='.$code);
        }
    ...
    public function _register($data, $OAuth = NULL, $groupid = NULL, $uid = NULL) {
        ...
        if ($OAuth) {
            ...
            // 注册送10金币
            $this->member_model->update_score(1, $uid, 10, '', '注册送10金币');
            ...
            if ($regverify == 1) {
                ...
                // 人工审核
                $this->admin_notice('member', fc_lang('新会员注册审核'), 'member/admin/home/index/field/uid/keyword/'.$uid);
            }
        }
        ...
        if (dr_is_app('invite') && $invite && ($member = $this->get_base_member($invite))) {
            ...
            if ($value['invite_experience']) {
                $this->update_score(0, $invite, (int)$value['invite_experience'], 'invite-'.$uid, "邀请好友注册奖励");
            }
            if ($value['invite_score']) {
                $this->update_score(1, $invite, (int)$value['invite_score'], 'invite-'.$uid, "邀请好友注册奖励");
            }
            // 邀请注册后的挂钩点
    ...
    public function setting($isdomain = FALSE) {
        ...
        foreach ($setting as $t) {
            if ($t['name'] == 'member') {
                $member = dr_string2array($t['value']);
            } else {
    ...
    public function guest($set = NULL) {
        ...
        if (!$data) {
            $this->db->insert('member_setting', array('name' => 'guest', 'value' => ''));
        }
        $data = dr_string2array($data['value']);
        // 修改数据
        if ($set) {
    ...
    public function cache() {
        ...
        if (SITE_FID) {
            $fenzhan_domain = require WEBPATH.'config/fenzhan.php'; // 加载分站域名配置文件
        } else {
            $fenzhan_domain = array();
        }
        ...
        foreach ($this->site_info as $sid => $t) {
            ...
            if ($t['SITE_MOBILE']) {
                // 移动端域名
                $cache['synurl'][] = dr_http_prefix($t['SITE_MOBILE']);
            }
            // 将站点的域名配置文件加入同步列表中
        ...
        // 空间域名
        if ($cache['setting']['space']['domain']) {
            $cache['synurl'][] = prep_url($cache['setting']['space']['domain']);
        }
    ...
    private function _where(&$select, $data) {
        ...
        if ($data) {
            ...
            if (isset($data['levelid']) && $data['levelid']) {
                $select->where('levelid', (int)$data['levelid']);
            }
        }

        // 判断groupid
        if (!isset($data['groupid']) && $_GET['groupid']) {
            $select->where('groupid', (int)$_GET['groupid']);
        }

        // 判断levelid
        if (!isset($data['levelid']) && $_GET['levelid']) {
            $select->where('levelid', (int)$_GET['levelid']);
        }
     ...
     * @return  array
     */
    public function limit_page($param, $page,$order='', $total) {
        ...
        $data = $select->order_by($order,'desc')->get('member')->result_array();
        $_param['total'] = $total;
        $_param['order'] = $order;
    ...
    public function update_score($type, $uid, $val, $mark, $note = '', $count = 0) {
        ...
        $value = $score + $val;
        $value = $value > 0 ? $value : 0; // 不允许积分或金币小于0
        unset($data);
        ...
        if ($type) {
            ...
            // 更新积分
            $this->db->where('uid', (int)$uid)->update('member', array('experience' => $value));
        }
        unset($value);
    ...
    public function add_sns($uid, $content, $attach = 0, $source = 0, $repost = 0) {
        ...
        // 给@的人发送提醒
        if ($user) {
            $this->add_notice($user, 2, fc_lang('【%s】在动态中@提到了我，<a href="%s" target="_blank">查看动态</a>。', $username, dr_sns_feed_url($uid, $id)));
        }

        // 给作者发送转发的提醒
        if ($repost) {
            $this->add_notice($row['uid'], 2, fc_lang('【%s】转发了我的动态，<a href="%s" target="_blank">查看动态</a>。', $username, dr_sns_feed_url($uid, $id)));
        }

        // 分数奖励
        if ($uid == $this->uid) {
            if ($this->member_rule['feed_experience']) {
                $this->update_score(0, $uid, (int)$this->member_rule['feed_experience'], '', "发布一条动态奖励");
            }
            if ($this->member_rule['feed_score']) {
                $this->update_score(1, $uid, (int)$this->member_rule['feed_score'], '', "发布一条动态奖励");
            }
        }
