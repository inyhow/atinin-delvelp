    ...
    private function _where(&$select, $data) {
        ...
        if ($data) {
            ...
            if (strlen($data['keyword']) > 0) {
                $select->where('(uid in (select uid from '.$this->db->dbprefix('member').' where `username`="'.$data['keyword'].'"))');
            }
    ...
    public function pay_success($sn, $money, $note = '') {
        ...
        if ($data['module']) {
            if ($data['module'] == 'order') {
                ...
                $order->link = $this->link = $this->site[SITE_ID];
                $order->member = $this->member;
                $order->pay($this->link->where('id IN ('.$data['order'].')')->get(SITE_ID.'_order')->result_array(), $money, $id);
            } else {
                // 其他模块
