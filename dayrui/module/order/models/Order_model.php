...
class Order_model extends CI_Model {
    ...
    public function notice($id, $order, $buy = 0, $sell = 0) {
        ...
        if (isset($cfg['email']) && $cfg['email']) {
            ...
            if ($buy && $m = dr_member_info($buy)) {
                $this->ci->sendmail_queue($m['email'], dr_lang('mod-073', $order['sn']), nl2br($content));
            }
            // 通知商家
    ...
    public function wc($order) {
        ...
        if ($order['order_price'] > 0 || $order['order_score'] >0) {
            if ($order['order_score']) {
                // 将金币打给卖家
                $this->member_model->update_score(1, $order['sell_uid'], intval($order['order_score']), '', 'lang,m-377,<a href="'.SITE_URL.'index.php?s=member&mod=order&c=home&m=info&id='.$order['id'].'" target="_blank">'.$order['sn'].'</a>');
            } else {
                // 将钱打给卖家
                ...
                $this->pay_model->add(
                    $order['sell_uid'],
                    $order['order_price'],
                    'lang,m-377,<a href="'.SITE_URL.'index.php?s=member&mod=order&c=home&m=info&id='.$order['id'].'" target="_blank">'.$order['sn'].'</a>'
                );
            }
        }

        // 入库商品索引表
        $goods = $this->link->where('oid', $order['id'])->get(SITE_ID.'_order_goods')->result_array();
        foreach ($goods as $t) {
            // 商品购买记录信息入库
            $this->link->insert(SITE_ID.'_order_buy', array(
                'mid' => $t['mid'],
                'cid' => $t['cid'],
                'oid' => $order['id'],
                'uid' => $order['buy_uid'],
            ...
            if (isset($this->mconfig['module'][$t['mid']]['volume']) && $this->mconfig['module'][$t['mid']]['volume']
                ...
                $this->link->where('id', (int)$t['cid'])->set('order_volume', 'order_volume+'.intval($t['quantity']), FALSE)->update(SITE_ID.'_'.$t['mid']);
    ...
    // 支付成功，付款扣钱
    public function pay($orders, $price, $pay_id = 0) {
        ...
        foreach ($orders as $order) {
            if ($order['order_price'] > 0 || $order['order_score'] > 0) {
                if ($order['order_score']) {
                    // 金币支付时
                    $pay_id = 0;
                    $this->member_model->update_score(1, $order['buy_uid'], -intval($order['order_price']), '', 'lang,m-200,<a href="'.SITE_URL.'index.php?s=member&mod=order&c=home&m=info&id='.$order['id'].'" target="_blank">'.$order['sn'].'</a>');
                } else {
                    // 扣钱
                    $this->load->model('pay_model');
            ...
            if (isset($this->mconfig['module'][$order['mid']]['volume']) && $this->mconfig['module'][$order['mid']]['volume']
                ...
                // 查询此订单的商品
                $goods = $this->link->where('oid', (int)$order['id'])->get(SITE_ID.'_order_goods')->result_array();
                foreach ($goods as $t) {
                    $this->link->where('id', (int)$t['cid'])->set('order_volume', 'order_volume+'.intval($t['quantity']), FALSE)->update(SITE_ID.'_'.$t['mid']);
                    $this->ci->clear_cache('show'.$t['mid'].SITE_ID.$t['cid']);
                    $this->hooks->call_hook('order_volume', array('good' => $t, 'order' => $order)); // 订单更新销量时的挂钩点
            ...
            // 付款日志
            $this->log($order, dr_lang('mod-062'));
            // 买家付款之后，提醒卖家
            ...
            if (isset($this->mconfig['module'][$order['mid']]['quantity'])
                ...
                $goods = $this->link->where('oid', $order['id'])->get(SITE_ID.'_order_goods')->result_array();
                foreach ($goods as $t) {
                    $row = $this->link->where('id', $t['cid'])->get(SITE_ID.'_'.$t['mid'])->row_array();
                    if ($row) {
                        $update = array();
                        if ($t['specification']) {
                            // sku 更新
                            $spec = dr_string2array($row['order_specification']);
                            ...
                            $update['order_specification'] = dr_array2string($spec);
                        }
                        $update['order_quantity'] = max(0, $row['order_quantity'] - $t['quantity']);
                        $this->link->where('id', $t['cid'])->update(SITE_ID.'_'.$t['mid'], $update);
                        $this->ci->clear_cache('show'.$t['mid'].SITE_ID.$t['cid']);
                    }
                }
    ...
    public function shopping($order, $data) {
        ...
        $this->link->where('id', $order['id'])->update(SITE_ID.'_order_data_'.$order['tableid'], array(
            'shipping_sn' => $order['shipping_sn'],
            'shipping_type' => $order['shipping_type'],
        ));

        // 记录日志
        $this->log($order, dr_lang('mod-064', $data['name'], $data['sn']));
        // 发货之后提醒买家
    ...
    public function log($order, $note) {
        ...
        $this->link->insert($this->prefix.'operate', array(
            'oid' => $order['id'],
            'uid' => $this->uid,
	...
	public function get_shipping_info($shipping, $city) {
		...
		$free = dr_string2array($shipping['free']);
        $freight = dr_string2array($shipping['freight']);
			...
			foreach ($store['goods'] as $t) {
				...
				if ($t['shipping'] && $city > 0) {
					...
					if (!isset($shipping[$sp]) || !$shipping[$sp]) {
						$shipping[$sp] = $this->link->where('id', $sp)->where('uid', $store['uid'])->get(SITE_ID.'_'.$t['mid'].'_shipping_'.(int)substr((string)$store['uid'], -1, 1))->row_array();
						if (!$shipping[$sp]) {
    ...
    public function check_goods($param, $goods) {
        ...
        if ($this->ci->mconfig['module'][$param['mid']]['quantity']) {
            if ( $goods['order_quantity'] < $param['quantity']) {
                return dr_lang('mod-034', $param['cid']);
            }
        }
	...
	public function get_goods($param) {
        ...
        if (!$param['cid'] || !$param['mid']) {
            return lang('mod-033'); // 参数不存在
        } elseif (!isset($this->ci->mconfig['module'][$param['mid']])
            || !$this->ci->mconfig['module'][$param['mid']]['use']) {
            return dr_lang('mod-023', $param['mid']); // 未开启电商功能
        }

        // 查询商品
        $data = $this->link->where('id', $param['cid'])->get(SITE_ID.'_'.$param['mid'])->row_array();
        if (!$data) {
            return dr_lang('mod-024', $param['cid']); // 商品不存在
        } elseif (isset($data['order_status']) && !$data['order_status']) {
            return dr_lang('mod-084', $param['cid']); // 商品下架
        }
        $data['thumb'] = $this->_get_thumb($data['thumb']);
        // 商品价格、库存计算
        if ($data['order_specification']) {
            $data['order_specification'] = dr_string2array($data['order_specification']);
	...
	public function get_cart_nums($uid, $agent) {
		...
		if (!$uid) {
			$this->link->where('agent', $agent);
		}
		$data = $this->link->select_sum('quantity')->get($this->prefix.'cart_'.(int)substr((string)$uid, -1, 1))->row_array();
	...
	public function add_cart($data) {
        ...
        if (!$data['uid'] && $data['agent']) {
            $this->link->where('agent', $data['agent']);
        }
		$cart = $this->link
					 ->where('uid', $data['uid'])
					 ->where('mid', $data['mid'])
					 ...
					 ->get($this->prefix.'cart_'.(int)substr((string)$data['uid'], -1, 1))
					 ->row_array();
		if ($cart) {
			$this->link->where('id', $cart['id'])->update($this->prefix.'cart_'.(int)substr((string)$data['uid'], -1, 1), array(
				'agent' => $data['agent'],
			...
			$this->link->replace($this->prefix.'cart_'.(int)substr((string)$data['uid'], -1, 1), $data);
		}
	...
	public function get_cart_row($uid, $id) {
		...
		return $this->link->where('id', $id)->get($this->prefix.'cart_'.(int)substr((string)$uid, -1, 1))->row_array();
	}
	...
	public function get_goods_info($data, $buy = 0) {
        ...
        /*
        if (is_dir(WEBPATH.'mall/')) {
            $this->load->add_package_path(WEBPATH.'mall/');
        ...
        foreach ($data as $t) {
            ...
            if (!isset($this->item[$mark])) {
                ...
                if (isset($row['order_stime']) && $row['order_stime'] && $row['order_stime'] > SYS_TIME) {
                    $check = 0; // 验证不通过
                }
                // 验证商品的结束有效期，结束时间小于当前时间，表示此商品销售时间到了
                if (isset($row['order_etime']) && $row['order_etime'] && $row['order_etime'] <= SYS_TIME) {
                    $check = 0; // 验证不通过
                }
                if ($row && $check){
                    $row['sku'] = array();
                    ...
                    // 购买步骤，按栏目设定的方法
                    $row['order_buy_step'] = (int)$this->ci->get_cache('module-'.SITE_ID.'-'.$t['mid'], 'category', $row['catid'], 'order_buy_step');
                    $row['order_specification'] = $row['order_specification'] ? dr_string2array($row['order_specification']) : '';
                    if ($row['order_specification'] && is_file(WEBPATH.$t['mid'].'/models/Spec_model.php')) {
                        $row['sku'] = dr_mall_spec($row, $row['order_specification']);
                    }
                    $row['order_score'] = (int)$this->ci->mconfig['module'][$t['mid']]['pay']; // 判断是否是金币支付
                    $this->item[$mark] = $row;
                } else {
                    // 删除无效商品
                    $this->link->where('id', $t['id'])->delete($this->prefix.'cart_'.(int)substr((string)$this->uid, -1, 1));
                    continue;
                }
            }
            if (!$this->item[$mark]) {
            ...
            $t['shipping_param'] = $this->item[$mark]['order_shipping_param'] ? dr_string2array($this->item[$mark]['order_shipping_param']) : 0;
            ...
            $t['order_score'] = (int)$this->ci->mconfig['module'][$t['mid']]['pay']; // 判断是否是金币支付
            ...
            if (!isset($list[$cate]['name'])) {
                ...
                $list[$cate]['price'] = 0;
                $list[$cate]['order_score'] = $t['order_score']; // 判断是否是金币支付
	...
	public function get_cart($uid, $agent) {
		...
		$data = $this->link->order_by('inputtime desc')->get($this->prefix.'cart_'.(int)substr((string)$uid, -1, 1))->result_array();
		if (!$data) {
			return NULL;
		}
    ...
    public function get_info_row($id) {
        ...
        $data = $this->link->where('id', $id)->get(SITE_ID.'_order')->row_array();
        if (!$data) {
            return 0; // 订单不存在
        }
	...
	public function get_info($id) {
        ...
        $temp = $this->link->where('id', $id)->get($this->prefix.'data_'.intval($data['tableid']))->row_array();
        if ($temp) {
            $data+= $temp;
        }

		$data['goods'] = $this->link->where('oid', $id)->get($this->prefix.'goods')->result_array();
     ...
     * @return	array
     */
    private function _where(&$select, $status, $transfer) {
        ...
        if ($data) {
			...
			if (isset($data['keyword']) && $data['keyword'] != '' && $data['field']) {
                ...
                if ($data['field'] == 'id') {
                    ...
                    $this->link->where('`id` IN (select oid from `'.$this->db->dbprefix(SITE_ID.'_order_goods').'` where `cid` = '.(int)$data['keyword'].')');
                } elseif ($data['field'] == 'title') {
                    $this->link->where('`id` IN (select oid from `'.$this->db->dbprefix(SITE_ID.'_order_goods').'` where `title` LIKE "%'.$data['keyword'].'%")');
                } else {
     ...
     * @return	array
     */
    public function limit_page($status, $transfer = 0) {
        ...
        if (!$total || IS_POST) {
            ...
            $_param = $this->_where($select, $status, $transfer);
