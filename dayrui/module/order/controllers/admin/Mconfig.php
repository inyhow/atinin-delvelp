...
class Mconfig extends M_Controller {
    ...
    public function __construct() {
        ...
        $this->setting = dr_string2array($mod['setting']);
    ...
    public function index() {
        ...
        if (IS_POST) {
            $this->setting[SITE_ID]['config'] = $this->input->post('data');
            $this->db->where('dirname', 'order')->update('module', array(
                'setting' => dr_array2string($this->setting)
            ));
            $this->admin_msg(fc_lang('操作成功'), dr_url('order/mconfig/index', array('page'=>$_POST['page'])), 1);
        }
        ...
        $this->template->assign(array(
			...
			'menu' => $this->get_menu_v3(array(
                lang('mod-016') => array('order/admin/mconfig/index', 'cog'),
                fc_lang('更新缓存') => array('admin/module/cache/dir/order', 'refresh')
            ))
        ));
    ...
    public function module() {
        ...
        if (IS_POST) {
            ...
            $this->admin_msg(fc_lang('操作成功'), dr_url('order/mconfig/'.$this->router->method, array('page'=>$_POST['page'])), 1);
        }
        ...
        $this->template->assign(array(
			...
			'menu' => $this->get_menu_v3(array(
                fc_lang('模块配置') => array('order/admin/mconfig/'.$this->router->method, 'cogs'),
                fc_lang('更新缓存') => array('admin/module/cache/dir/order', 'refresh')
            )),
            'module' => $module,
        ));
    ...
    public function permission() {
		...
		$group[0] = array('name' => fc_lang('游客'), 'id' => 0);
		asort($group);
        ...
        $this->template->assign(array(
			...
			'menu' => $this->get_menu_v3(array(
                lang('mod-017') => array('order/admin/mconfig/'.$this->router->method, 'user'),
                fc_lang('更新缓存') => array('admin/module/cache/dir/order', 'refresh'),
            )),
			'group' => $group,
        ));
    ...
    public function paytype() {
        ...
        $this->template->assign(array(
            ...
            'data' => $this->setting[SITE_ID][$this->router->method],
			'menu' => $this->get_menu_v3(array(
                lang('mod-046') => array('order/admin/mconfig/'.$this->router->method, 'rmb'),
                fc_lang('更新缓存') => array('admin/module/cache/dir/order', 'refresh'),
            )),
        ));
        $this->template->display('mconfig_'.$this->router->method.'.html');
    ...
    public function notice() {
        ...
        $this->template->assign(array(
            ...
            'data' => $this->setting[SITE_ID][$this->router->method],
			'menu' => $this->get_menu_v3(array(
                lang('mod-072') => array('order/admin/mconfig/'.$this->router->method, 'volume-off'),
                fc_lang('更新缓存') => array('admin/module/cache/dir/order', 'refresh'),
            )),
        ));
        $this->template->display('mconfig_'.$this->router->method.'.html');
