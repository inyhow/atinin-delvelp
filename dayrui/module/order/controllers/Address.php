...
class Address extends M_Controller {
    ...
    public function index() {
        ...
        if ($this->uid) {
            ...
            $data = dr_string2array($this->session->userdata('guest_address'));
            if ($data) {
                $list = array( 0 => $data );
                $default = $list[0];
            }
    ...
    public function add() {
		...
		if ($this->input->get('submit')) {
			...
			if (!$data['city'] || !$data['name'] || !$data['phone'] || !$data['zipcode'] || !$data['address']) {
                $this->return_jsonp(dr_json(0, lang('mod-020')));
			} else {
                if ($this->uid) {
                    $this->address_model->add_address($data);
                } else {
                    $this->session->set_userdata('guest_address', dr_array2string($data));
                }
