...
class D_Admin_Home extends M_Controller {
    ...
    public function _get_field($catid = 0) {
        ...
        if ($category) {
            $field = array_merge($field, $category);
        }
        // 筛选出右边显示的字段
    ...
    public function index() {
        ...
        if (IS_POST && !$this->input->post('search')) {
            ...
            switch ($action) {
                case 'del':
                    $ok = $no = 0;
                    foreach ($ids as $id) {
                        $data = $this->link->where('id', (int)$id)->select('id,catid,tableid')->get($this->content_model->prefix)->row_array();
                        if ($data) {
                ...
                case 'order':
                    ...
                    foreach ($ids as $id) {
                        $this->link->where('id', $id)->update($this->content_model->prefix, $_data[$id]);
                    }
        ...
        $_menu = array();
        // 统计推荐位数量
        $_menu[fc_lang('已通过的内容')] = array($catid ? APP_DIR.'/admin/home/index/catid/'.$catid : APP_DIR.'/admin/home/index', 'table');
        $flag = $this->get_cache('module-'.SITE_ID.'-'.APP_DIR, 'setting', 'flag');
        if ($flag) {
            foreach ($flag as $id => $t) {
                if ($t['name'] && $id) {
                    $_menu["{$t['name']}(".$this->content_model->flag_total($id, $catid).")"] = array($catid ?
                            APP_DIR.'/admin/home/index/flag/'.$id.'/catid/'.$catid :
                            APP_DIR.'/admin/home/index/flag/'.$id, 'flag');
                } else {
					unset($flag[$id]);
				}
            }
        ...
        $this->template->assign(array(
            ...
            'post' => $catid ? APP_DIR.'/admin/home/add/catid/'.$catid : APP_DIR.'/admin/home/add',
            'flags' => $flag,
            'param' => $param,
            'field' => $this->field,
            ...
            if (isset($data['error'])) {
                ...
                if (($id = $this->content_model->add($data, $syncatid)) != FALSE) {
                    ...
                    if ($rule['experience'] + $member['experience'] >= 0) {
                        $this->member_model->update_score(0, $uid, $rule['experience'], $mark, "发布文档", 1);
                    }
                    // 金币处理
                    if ($rule['score'] + $member['score'] >= 0) {
                        $this->member_model->update_score(1, $uid, $rule['score'], $mark, "发布文档", 1);
                    }
                    // 操作成功处理附件
    ...
    public function edit() {
        ...
        if (IS_POST) {
            ...
            if (isset($post['error'])) {
                ...
                // 发布草稿时删除草稿数据
                if ($did && $this->content_model->delete_draft($did, 'cid='.$lid.' and eid=0')) {
                    $this->attachment_replace_draft($did, $lid, 0, $this->content_model->prefix);
                }
                // 操作成功处理附件
                ...
                if ($update !== $myflag) {
                    // 删除旧的
                    if ($myflag) {
                        $this->link->where('id', $id)->where_in('flag', $myflag)->delete(SITE_ID.'_'.APP_DIR.'_flag');
                    }
                    // 增加新的
                    if ($update) {
                        foreach ($update as $i) {
    ...
    public function verify() {
        ...
        if ($this->admin['adminid'] > 1 && !$this->verify) {
            $this->admin_msg(fc_lang('此角色组还没有分配审核流程'));
        }
            ...
            switch ($this->input->post('action')) {
                ...
                case 'flag': // 标记
                    ...
                    if ($error) {
                        ...
                        exit(dr_json(2, fc_lang('操作成功，正在刷新...'), $js));
                    }
        ...
        if ($this->admin['adminid'] == 1) {
            ...
            for ($i = 0; $i < 9; $i++) {
                ...
                $key_name = ($i ? fc_lang('%s审', $i) : fc_lang('被退回')).' ('.$total.')';
                if ($total && !$goto && $i > 0) {
                    if ($param['status'] == $i) {
                        $goto = 1;
                    } else {
                ...
                $_menu[$key_name] = array(APP_DIR.'/admin/home/verify'.(isset($_GET['status']) || $i ? '/status/'.$i : ''), 'reorder');
            }
            // 跳转到对应的状态
            if (strlen($goto) > 1 && isset($_menu[$goto])) {
                redirect(ADMIN_URL.$this->duri->uri2url($_menu[$goto][0]), 'refresh');
                exit;
            }
        } else {
            // 非管理员角色列出自己审核的
        ...
        if ($this->admin['adminid'] > 1) {
            // 被退回
            $_total = $this->link->where('`status`=0 AND `backuid`='.$this->uid)->count_all_results($this->content_model->prefix.'_verify');
            $_menu[fc_lang('被退回').' ('.$_total.')'] = array(APP_DIR.'/admin/home/verify', 'reply');
            // 我的审核
            $_total = $this->link->where_in('status', $status)->count_all_results($this->content_model->prefix.'_verify');
            $_menu[fc_lang('我的审核').' ('.$_total.')'] = array(APP_DIR.'/admin/home/verify/status/1', 'user');
        }
    ...
    public function _verify($id, $data, $_where) {
        ...
        if ($this->input->post('flagid') > 0) {
            ...
            if ($status == 9) {
                ...
                // 积分处理
                if ($rule['experience']) {
                    $this->member_model->update_score(0, $verify['uid'], $rule['experience'], $mark, "发布文档", 1);
                }
                // 金币处理
                if ($rule['score']) {
                    // 金币判断重复
        ...
        if ($this->input->get('ispost')) {
            ...
            if ($this->input->get('tab') == 1) {
                ...
                if (!$value) {
				 ...
				 // 执行同步指定栏目
				foreach ($ids as $id) {
					$this->link->where('id', (int)$id)->update($this->content_model->prefix, array('link_id' => -1)); // 更改状态
					$data = $this->link->where('id', (int)$id)->get($this->content_model->prefix)->row_array(); // 获取数据
					if (!$data) {
						continue;
					}
					foreach ($value as $catid) {
						if ($catid && $catid != $data['catid']) {
							...
							$this->link->replace($this->content_model->prefix, $new[1]); // 创建主表
        ...
        if ($this->input->get('todo')) {
            ...
            foreach ($data as $t) {
                if ($t['link_id'] && $t['link_id'] >= 0) {
                    // 同步栏目的数据
                    $i = $t['id'];
                    $t = $this->link->where('id', (int)$t['link_id'])->get($table)->row_array();
                    if (!$t) {
                        continue;
                    }
                ...
                if ($module['extend']) {
                    ...
                    if ($extend) {
                        foreach ($extend as $e) {
                            ...
                            $this->link->where('id=',(int)$e['id'])->update($table.'_extend', array(
