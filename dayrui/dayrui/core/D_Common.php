...
class D_Common extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        // 检测到未安装
        if (!is_file(WEBPATH.'cache/install.lock')) {
        ...
        //判断是否实名认证
        /*$auth = $this->db->where('uid',$this->uid)->get('member_data')->row_array();
        $is_auth = $auth['is_auth'];*/

        //判断会员所在分组、用户名
        $member = $this->db->where('uid', $this->uid)->get('member')->row_array();
        ...
        // 分站信息
        if (SITE_FID) {
            $this->template->assign('fenzhan', $this->get_cache('fenzhan', SITE_ID, 'value', SITE_FID));
        }
    }
    ...
    public function get_cache() {
        ...
        if (isset($param[0])
            ...
            && is_file(FCPATH.'dayrui/models/'.ucfirst($param[0]).'.php')) {
            $model = strtolower(array_shift($param));
        }
            ...
            if (isset($this->$var) && $this->$var) {
                ...
                if ($data === false) {
                    ...
                    if (strpos($name, 'navigator') === 0) {
                        // 网站导航缓存
                        list($a, $siteid) = explode('-', $name);
                        $this->load->model('navigator_model');
                        $data = $this->navigator_model->cache($siteid ? $siteid : SITE_ID);
                    } elseif (strpos($name, 'poster-code') === 0) {
                        // 网站广告缓存(视频播放)
                        if (dr_is_app('adm')) {
                            $this->load->add_package_path(FCPATH.'app/adm/');
                            list($a, $b, $siteid) = explode('-', $name);
                            $this->load->model('poster_model');
                            $data = $this->poster_model->cache($siteid ? $siteid : SITE_ID, 'code');
                        } else {
                            return array();
                        }
                    } elseif (strpos($name, 'poster-video') === 0) {
                        // 网站广告缓存(视频播放)
                        if (dr_is_app('adm')) {
                            $this->load->add_package_path(FCPATH.'app/adm/');
                            list($a, $b, $siteid) = explode('-', $name);
                            $this->load->model('poster_model');
                            $data = $this->poster_model->cache($siteid ? $siteid : SITE_ID, 'video');
                        } else {
                            return array();
                        }
                    } elseif (strpos($name, 'poster-text') === 0) {
                        // 网站广告缓存(视频滚动文字)
                        if (dr_is_app('adm')) {
                            $this->load->add_package_path(FCPATH.'app/adm/');
                            list($a, $b, $siteid) = explode('-', $name);
                            $this->load->model('poster_model');
                            $data = $this->poster_model->cache($siteid ? $siteid : SITE_ID, 'text');
                        } else {
                            return array();
                        }
                    } elseif (strpos($name, 'poster') === 0) {
                        // 网站广告缓存
                        if (dr_is_app('adm')) {
                            $this->load->add_package_path(FCPATH.'app/adm/');
                            list($a, $siteid) = explode('-', $name);
                            $this->load->model('poster_model');
                            //$data = $this->poster_model->cache($siteid ? $siteid : SITE_ID);
                        } else {
                            return array();
                        }
                    ...
                    } elseif (strpos($name, 'sysvar') === 0) {
                        // 全局变量
                        $this->load->model('system_model');
                        $data = $this->system_model->urlrule();
                    } elseif (strpos($name, 'comment') === 0) {
                        // 评论
                        $this->load->model('system_model');
                        $data = $this->system_model->comment();
                    } elseif (strpos($name, 'attachment') === 0) {
                        // 远程附件 规则
                        $this->load->model('system_model');
                        $data = $this->system_model->attachment();
                    ...
                    } elseif (strpos($name, 'form-name-') === 0) {
                        // 网站表单名称id缓存
                    ...
                    } elseif (strpos($name, 'linkid-') === 0) {
                        // 联动菜单ID缓存
                        list($a, $site) = explode('-', $name);
                        $this->load->model('linkage_model');
                        $this->linkage_model->cache($site);
                        $data = $this->dcache->get('linkid-'.$site);
                    ...
                    } elseif (strpos($name, 'branch-') === 0) {
                        // 分支系统缓存
                        list($a, $model) = explode('-', $name);
                        $this->load->model($model.'_model', 'my_branch');
                        $data = $this->my_branch->cache();
                    } elseif ($model) {
                        // 存在模型时加载模型缓存
                        $this->load->model($model, 'my_model');
                        $data = $this->my_model->cache();
                    } else {
                        //var_dump($name);
                    }
    ...
    public function get_cache_data($name) {
        ...
        if (defined('SYS_MEMCACHE') && SYS_MEMCACHE && $this->cache->memcached->is_supported()) {
    ...
    private function _init_variable() {
        ...
        foreach ($sitecfg as $file) {
            ...
            if (is_file(WEBPATH.'config/site/'.$file)) {
                ...
                $this->site_info[$id]['SITE_URL'] = dr_http_prefix(($this->site_info[$id]['SITE_DOMAIN'] ? $this->site_info[$id]['SITE_DOMAIN'] : DOMAIN_NAME).SITE_PATH);
            }
        }
        unset($sitecfg);
        ...
        if ($siteid && isset($this->site_info[$siteid])) {
            ...
            if ($orthers && DOMAIN_NAME != $this->site_info[$siteid]['SITE_DOMAIN'] && in_array(DOMAIN_NAME, $orthers)) {
                ...
                if (IS_ADMIN) {
                    ...
                    redirect(dr_http_prefix($this->site_info[$siteid]['SITE_DOMAIN'].'/'.$uri), '', '301');
                    exit;
                }
        ...
        $this->mark = @end($config2);
        $this->mark = strlen($this->mark) > 10 ? $this->mark : NULL;
        ...
        $config3['UEDITOR_IMG_ID'] = $this->mark ? md5($config3['SYS_ATTACHMENT_URL']) : '")'; // 编辑器附件ID
        ...
        $config3['SYS_HELP_URL'].= 'index.php?c=show&fc='.$config3['DR_LICENSE_ID'].'&domain='.$config3['SITE_URL'].'&id=';
        // 将配置文件数组转换为常量
        ...
        // 管理员登陆才显示数据库错误提示
        $this->db->db_debug = $this->site[SITE_ID]->db_debug = $this->member['adminid'] || IS_ADMIN ? TRUE : FALSE;
        ...
        if (IS_MEMBER) {
            ...
            // 当模块下不存在左侧栏目时默认选中列表页
            $cur = !$cur && APP_DIR && (is_dir(FCPATH.'module/'.APP_DIR) || is_dir(FCPATH.'app/'.APP_DIR)) && isset($menu['uri'][APP_DIR.'/home/index']) ? $menu['uri'][APP_DIR.'/home/index'] : $cur;
            ...
            // 登录判断
            if (!in_array($this->router->class, array('register', 'login', 'api', 'sns'))) {
                $uri = $this->router->class.'-'.$this->router->method;
                $url = dr_member_url('login/index', array('backurl' => urlencode(dr_now_url())));
                // 支付回调页面不验证
                $verify = defined('DR_PAY_ID') && DR_PAY_ID ? FALSE : TRUE;
                // 会员中心不验证
                $verify = (!APP_DIR && $this->router->class == 'home' && $this->router->method == 'index') || ($this->router->class == 'home' && $this->router->method == 'geetest') ? FALSE : $verify;
                // 游客发布权限不验证
                $verify = APP_DIR && in_array($uri, array('home-add', 'home-field')) && !$this->member ? FALSE : TRUE;
                // 待审核会员组
                $this->member['groupid'] == 1 && $uri != 'home-index' && $this->member_msg(fc_lang('对不起，您还没有通过审核，无法进行此操作'));
                // 没有登录时
                $verify && !$this->uid && $this->member_msg(fc_lang('会话超时，请重新登录').$this->member_model->logout(), $url);
                // 会员不存在时
                $verify && !$this->member && $this->member_msg(fc_lang('会员不存在，请重新登录').$this->member_model->logout(), $url);
                ##
                if ($this->uid) {
                    ...
                    if ((APP_DIR || ($uri != 'home-index' && $uri != 'pm-webchat'))
                        && (!defined('DR_UEDITOR') && !defined('DR_PAY_ID') && !defined('DISCUZ_ROOT'))) {
            ...
            if (!$this->is_auth($uri)) {
                if (IS_AJAX) {
                    exit(fc_lang('您无权限操作(%s)', $uri));
                } else {
                    $this->admin_msg(fc_lang('您无权限操作(%s)', $uri));
                }
            }
            $share = 0;
            if ($MOD) {
    ...
    protected function get_menu_v3($menu) {
        ...
        if (!$_cur && $this->router->method == 'edit') {
            $_link.= '<li><a href="javascript:;" class="'.$this->admin['color'].'"><i class="fa fa-edit"></i> '.fc_lang('修改').'</a> <i class="fa fa-circle"></i> ';
            $_cur = fc_lang('修改');
        }

        if (!$_cur) {
            $_link = str_replace('{MARK}', $this->admin['color'], $_link);
            if (!$_name) {
                $_name = $_first;
            }
        } else {
    ...
    public function get_member_pagination($url, $total) {
        ...
        $config = array();
        if (is_file(FCPATH.'module/member/config/pagination.php')) {
            include FCPATH.'module/member/config/pagination.php';
        } else {
    ...
    protected function is_admin_login() {
        ...
        $data['color'] = $data['color'] ? $data['color'] : 'blue';
        return $data;
    }
    ...
    protected function validate_filter($_field, $_data = array()) {
        ...
        foreach ($_field as $field) {
            ...
            if ($field['ismain']
                ...
                if ($this->site[SITE_ID]->where('id<>', (int)$_data['id'])->where($name, $this->post[$name])->count_all_results(SITE_ID.'_'.APP_DIR)) {
...
{value}';
    ...
    protected function api_favorite() {
        ...
        $eid = $cid ? $id : 0; // 扩展表id
        $data = $this->link->where('id', $mid)->select('url,title')->get(SITE_ID.'_'.APP_DIR)->row_array();
        if (!$data) {
            // 文档不存在
            if (isset($_GET['jsonp']) && $_GET['jsonp']) {
        ...
        $favorite = $this->link->where('cid', $mid)->where('uid', $this->uid)->where('eid', $eid)->select('id')->get($table)->row_array();
        if ($eid) {
            // 收藏扩展表
            $data2 = $this->link->where('cid', $mid)->get(SITE_ID.'_'.APP_DIR.'_extend')->row_array();
            if ($favorite) {
                $this->link->where('id', $favorite['id'])->update($table, array(
                    'url' => $data2['url'],
                ...
                // 更新数量
                $c = $this->link->where('eid', $eid)->count_all_results($table);
                $this->link->where('id', $eid)->set('favorites', $c)->update(SITE_ID.'_'.APP_DIR.'_extend');
                if (isset($_GET['jsonp']) && $_GET['jsonp']) {
            ...
            if ($favorite) {
                ...
                // 更新数量
                $c = $this->link->where('cid', $mid)->count_all_results($table);
                $this->link->where('id', $mid)->set('favorites', $c)->update(SITE_ID.'_'.APP_DIR);
                if (isset($_GET['jsonp']) && $_GET['jsonp']) {
                    echo $this->input->get('callback', TRUE).'('.json_encode(array('code' => '4')).')';exit;
                } else {
                    exit('4');
    ...
    public function goto_404_page($msg) {
        ...
        $root = WEBPATH.'dayrui/'.($this->template->mobile ? 'mobiles' : 'templates').'/'.SITE_TEMPLATE.'/404.html';
    ...
    public function s404() {
        if ($this->member['adminid'] == 1) {
            $this->goto_404_page('无法找到对应的页面('.DR_URI.'),可能您没有配置好路由规则');
        } else {
            $this->_indexc();
        }
    }
        ...
        if ($t == 1) {
            ...
            if ($pay['order']) {
                foreach ($pay['order'] as $dir => $v) {
                    if (@in_array($dir, $pay['use'])) {
                        if (is_file(WEBPATH.'api/pay/'.$dir.'/config.php')) {
                            $data[$dir] = require WEBPATH.'api/pay/'.$dir.'/config.php';
                        }
                    }
                }
            }
    ...
    protected function _indexc() {
        ...
        /*is_file(FCPATH.'dayrui/helpers/function_common.php') && include FCPATH.'dayrui/helpers/function_common.php';*/
        //is_file(FCPATH.'dayrui/helpers/function_cal.php') && include FCPATH.'dayrui/helpers/function_cal.php';
        //$aa = interest(45000);
        //print_r($aa);

        // 系统开启静态首页、静态文件不存在时，才生成文件
        if (SYS_CACHE_INDEX && !is_file($file) && !SITE_CLOSE) {
            ob_start();
            $this->template->assign(array(
                'indexc' => 1,
    ...
    protected function _module_init() {
        ...
        // 设置模块模板
        $this->template->module($module['template'] && is_dir(FCPATH.'module/'.$this->dir.'/templates/'.$module['template'].'/') ? $module['template'] : 'default');
        ...
        if (defined('SYS_UPDATE') && SYS_UPDATE) {
            ...
            foreach ($syn as $t) {
                @copy(WEBPATH.'cache/update/module'.$t, APPPATH.'controllers'.$t);
            }
        }
    }
