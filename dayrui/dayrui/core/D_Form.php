...
class D_Form extends M_Controller {
	...
	protected function _listc() {
		...
		if (!$this->is_auth('admin/form/listc')) {
			if (IS_AJAX) {
				exit(fc_lang('抱歉！您无权限操作(%s)', 'admin/form/listc'));
			} else {
				$this->admin_msg(fc_lang('抱歉！您无权限操作(%s)', 'admin/form/listc'));
			}
		}
		...
		$this->template->assign(array(
			...
			'menu' => $this->get_menu_v3(array(
				...
				$this->form['name'] => array($this->uriprefix.'index', $this->form['setting']['icon'] ? str_replace('fa fa-', '', $this->form['setting']['icon']) : 'table'),
    ...
    protected function _post() {
		...
		if (!$this->form['setting']['post']) {
			if (IS_POST) {
				exit($this->call_msg(fc_lang('此表单没有开启前端提交功能')));
			} else {
				$this->msg(fc_lang('此表单没有开启前端提交功能'));
			}
		}
		...
		if (IS_POST) {
			...
			if ($this->form['setting']['code'] && !$this->check_captcha('code')) {
                exit($this->call_msg(fc_lang('验证码不正确')));
            }
