...
class D_Page extends M_Controller {
    ...
    protected function _get_page($id, $dir, $pid) {
        ...
        if (!$id && !$dir) {
            $this->goto_404_page(fc_lang('参数不完整，id与dir必须有一个'));
        }
        ...
        $data = $page[$id];
        if (!$data) {
            $this->goto_404_page(fc_lang('单网页（%s）不存在', $id));
        }
	...
	protected function _page() {
        ...
        // 生成缓存
        file_put_contents(WEBPATH.'cache/page/'.md5(PAGE_CACHE_URL).'.html', $html, LOCK_EX);
		...
		if ($data) {
			foreach($data as $t) {
				...
				if ($this->is_auth(APP_DIR.'/admin/page/add')) {
					$t['option'].= '<a class="aadd" href='.dr_url(APP_DIR.'/page/add', array('id' => $t['id'])).'> <i class="fa fa-plus"></i> '.fc_lang('添加子页').'</a>';
				}
				if ($this->is_auth(APP_DIR.'/admin/page/edit')) {
					$t['option'].= '<a class="aedit" href='.dr_url(APP_DIR.'/page/edit', array('id' => $t['id'])).'> <i class="fa fa-edit"></i> '.fc_lang('修改/内容').'</a>';
				}
