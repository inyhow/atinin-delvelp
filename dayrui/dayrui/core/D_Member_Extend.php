...
class D_Member_Extend extends M_Controller {
    ...
    public function __construct() {
        ...
        if ($this->router->method != 'buy' && !$this->content) {
    ...
    public function index() {
        ...
        if (!$total) {
            ...
            $this->link->where('uid', $this->uid)->where('cid', (int)$this->content['id'])->where('status>=9');
            $data = $this->link->get(SITE_ID.'_'.APP_DIR.'_extend')->row_array();
        ...
        if ($total) {
            ...
            $list = $this->link->limit($this->pagesize, $this->pagesize * ($page - 1))->get(SITE_ID.'_'.APP_DIR.'_extend')->result_array();
        }
    ...
    public function add() {
        ...
        // 添加权限
        if (!$this->catrule['add']) {
            $this->member_msg(fc_lang('您无权限操作'));
        }

        // 金币检查
        if ($this->catrule['extend_score'] + $this->member['score'] < 0) {
            $this->member_msg(fc_lang(SITE_SCORE.'不足！本次需要%s'.SITE_SCORE.'，当前余额%s'.SITE_SCORE, abs($this->catrule['extend_score']), $this->member['score']));
        }
                ...
                if ($id = $this->content_model->add_extend($data)) {
                    ...
                    if ($data[1]['status'] >= 9) {
                        ...
                        if ($this->catrule['extend_experience']) {
                            $this->member_model->update_score(0, $this->content['uid'], $this->catrule['extend_experience'], $mark, fc_lang('发布内容'), 1);
                        }
                        // 金币处理
                        if ($this->catrule['extend_score']) {
                            $this->member_model->update_score(1, $this->content['uid'], $this->catrule['extend_score'], $mark, fc_lang('发布内容'), 1);
                        }
                        // 操作成功处理附件
                        $this->attachment_handle($this->content['uid'], $mark, $this->field);
                        if (IS_AJAX || IS_API_AUTH) {
                            exit(dr_json(1, fc_lang('发布成功，马上返回列表'), dr_member_url(APP_DIR.'/extend/index', array('cid' => $this->content['id'])), $id));
                        }
                        $this->template->assign(array(
                        ...
                        if (IS_AJAX || IS_API_AUTH) {
                            exit(dr_json(1, fc_lang('发布成功，请等待管理员审核'), dr_member_url(APP_DIR . '/everify/index'), $id));
                        }
                        $this->template->assign(array(
    ...
    public function buy() {
        ...
        $table = $this->link->dbprefix(SITE_ID.'_'.APP_DIR.'_extend_buy');
