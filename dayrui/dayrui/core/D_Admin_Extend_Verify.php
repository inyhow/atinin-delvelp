    ...
    public function index() {
        ...
        if ($this->admin['adminid'] == 1) {
            ...
            for ($i = 0; $i < 9; $i++) {
                ...
                $_menu[$key_name] = array(APP_DIR.'/admin/verify/index'.(isset($_GET['status']) || $i ? '/status/'.$i : ''), 'flag');
            }
			// 跳转到对应的状态
        ...
        if ($this->admin['adminid'] > 1) {
            // 被退回
            $_total = $this->link->where('`status`=0 AND `backuid`='.$this->uid)->count_all_results($this->table);
            $_menu[fc_lang('被退回').' ('.$_total.')'] = array(APP_DIR.'/admin/verify/index', 'user');
            // 我的审核
            $_total = $this->link->where_in('status', $status)->count_all_results($this->table);
            $_menu[fc_lang('我的审核').' ('.$_total.')'] = array(APP_DIR.'/admin/verify/index/status/1', 'flag');
        }
    ...
    public function _verify($id, $data, $_where) {
        ...
        if ($this->input->post('flagid') > 0) {
            ...
            if (!$data) {
                ...
                if (isset($data[1]['mytype'])) {
                    $data[1]['mytype'] = $verify['content']['mytype'];
                }
            }
            ...
            // 拒绝审核
            // 更改主表状态
            $this->link->where($_where)->update($this->content_model->prefix.'_extend', array('status' => 0));
            // 更改索引表状态
            $this->link->where($_where)->update($this->content_model->prefix.'_extend_index', array('status' => 0));
            // 更改审核表状态
            $this->link->where($_where)->update($this->content_model->prefix.'_extend_verify', array(
                'status' => 0,
