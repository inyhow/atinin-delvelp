...
class D_Member_Form extends M_Controller {
    ...
    public function index() {
		...
		if (!$total) {
			...
			$data = $this->link->get($this->table)->row_array();
		...
		if ($total) {
			...
			$list = $this->link->limit($this->pagesize, $this->pagesize * ($page - 1))->get($this->table)->result_array();
		}
		...
		$this->template->display(is_file(APPPATH.'templates/member/'.MEMBER_TEMPLATE.'/mform_index_'.$this->form['table'].'.html') || is_file(FCPATH.'templates/member/'.MEMBER_TEMPLATE.'/mform_index_'.$this->form['table'].'.html') ? 'mform_index_'.$this->form['table'].'.html' : 'mform_index.html');
    }
    ...
    public function edit() {
		...
		$data = $this->link->where('id', $id)->where('uid', $this->uid)->get($this->table)->row_array();
		if (!$data) {
            $this->admin_msg(fc_lang('表单内容(id#%s)不存在', $id));
        }

        $data2 = $this->link->where('id', $id)->get($this->table.'_data_'.intval($data['tableid']))->row_array();
        if ($data2) {
		...
		if (IS_POST) {
			...
			if (isset($data['error'])) {
				...
				$this->link->where('id', $id)->update($table, $post[1]);
                $this->link->where('id', $id)->update($table.'_data_'.intval($data['tableid']), $post[0]);
				// 操作成功处理附件
				$this->attachment_handle($data['uid'], $table.'-'.$id, $this->form['field'], $post);
                if (IS_AJAX || IS_API_AUTH) {
                    exit(dr_json(0, 'ok', '', $id));
                }
				$this->admin_msg(fc_lang('操作成功，正在刷新...'), dr_member_url(APP_DIR.'/'.$this->router->class.'/index'), 1);
			}
		}
		
		$tpl = APPPATH.'templates/member/'.MEMBER_TEMPLATE.'/mform_edit_'.$this->form['table'].'.html';
		$this->template->assign(array(
			'data' => $data,
			'error' => $error,
    ...
    public function del() {
        ...
        $result = $this->link->where('id', $id)->where('uid', $this->uid)->get($table)->row_array();
		if ($result) {
            // 更新模块表的统计值
            $this->link->where('id', $result['cid'])->set($this->form['table'].'_total', $this->form['table'].'_total - 1', FALSE)->update(SITE_ID.'_'.APP_DIR);
            // 删除
            $this->link->where('id', $id)->delete($table);
            $this->link->where('id', $id)->delete($table.'_data_'.$result['tableid']);
    ...
    public function show() {
		...
		$id = (int)$this->input->get('id');
		$data = $this->link->where('id', $id)->get($this->table)->row_array();
		if (!$data) {
            exit('<div style="padding:10px 20px 20px;">'.fc_lang('表单内容(id#%s)不存在', $id).'</div>');
        }
		...
		$tpl = APPPATH.'templates/member/'.MEMBER_TEMPLATE.'/mform_show_'.$this->form['table'].'.html';
		$this->template->assign(array(
