...
class D_Member_Home extends M_Controller {
    ...
    public function index() {
        ...
        if (IS_POST) {
            ...
            if ($this->input->post('action') == 'update') {
                ...
                if ($this->member_rule['update_score'] + $this->member['score'] < 0) {
                    exit(dr_json(0, dr_lang('抱歉，'.SITE_SCORE.'不足！本次需要%s'.SITE_SCORE.'，当前余额%s'.SITE_SCORE, abs($this->member_rule['update_score']), $this->member['score'])));
                }
                // 积分检查
                ...
                if (!isset($this->flag[$i])) {
                    exit(dr_json(0, fc_lang('推荐位不存在或者您无权操作')));
                }
                $count = count($ids);
                $value = abs($this->flag[$i][$this->markrule] * $count);
                // 金币检查
                if ($this->member['score'] - $value < 0) {
                    exit(dr_json(0, fc_lang(SITE_SCORE.'不足！本次需要%s'.SITE_SCORE.'，当前余额%s'.SITE_SCORE, $value, $this->member['score'])));
                }
        ...
        if (!$total) {
            ...
            $this->link->where('uid', $this->uid)->where_in('catid', $this->catid)->where('status>=9')->where('link_id<1');
            // 搜索关键字
            if ($kw) {
            ...
            $data = $this->link->get($this->content_model->prefix)->row_array();
        ...
        if ($total) {
            ...
            $list = $this->link->limit($this->pagesize, $this->pagesize * ($page - 1))->get($this->content_model->prefix)->result_array();
        }
       ...
       // $score = $flag[$id][$this->member['mark']];
        ...
        if (IS_POST) {
            ...
            if ($this->input->post('action') == 'update') {
                // 金币检查
                if ($this->member_rule['update_score'] + $this->member['score'] < 0) {
                    exit(dr_json(0, dr_lang(SITE_SCORE.'不足！本次需要%s'.SITE_SCORE.'，当前余额%s'.SITE_SCORE, abs($this->member_rule['update_score']), $this->member['score'])));
                }
                // 积分检查
    ...
    public function add() {
        ...
        if (IS_POST) {
            ...
            if ($this->uid && $this->module_rule[$catid]['postnum']) {
                ...
                if ($total >= $this->module_rule[$catid]['postnum']) {
                    $this->member_msg(fc_lang('每日发布数量不得超过%s个', $this->module_rule[$catid]['postnum']));
                }
            }
            // 投稿总数检查
            if ($this->uid && $this->module_rule[$catid]['postcount']) {
                ...
                if ($total >= $this->module_rule[$catid]['postcount']) {
                    $this->member_msg(fc_lang('发布总数不得超过%s个', $this->module_rule[$catid]['postcount']));
                }
            }
            // 金币检查
            if ($this->uid && $this->module_rule[$catid]['score'] + $this->member['score'] < 0) {
                $this->member_msg(fc_lang(SITE_SCORE.'不足！本次需要%s'.SITE_SCORE.'，当前余额%s'.SITE_SCORE, abs($this->module_rule[$catid]['score']), $this->member['score']));
            }
            // 字段验证与过滤
            ...
            if (isset($data['error'])) {
                ...
                if ($this->input->post('action') == 'draft') {
                    ...
                    if (IS_AJAX || IS_API_AUTH) {
                        exit(dr_json(0, fc_lang('已保存到我的草稿箱中'), dr_member_url(APP_DIR.'/home/draft/'), $id));
                    }
                ...
                if (($id = $this->content_model->add($data)) != FALSE) {
                    ...
                    if ($data[1]['status'] >= 9) { // 审核通过
                        ...
                        if ($experience) {
                            $this->member_model->update_score(0, $this->uid, $experience, $mark, "发布文档", 1);
                        }
                        // 金币处理
                        $score = (int) $this->module_rule[$catid]['score'];
                        if ($score) {
                            $this->member_model->update_score(1, $this->uid, $score, $mark, "发布文档", 1);
                        }
                        // 附件归档到文档
                        $this->attachment_handle($this->uid, $mark, $field);
                        $this->attachment_replace($this->uid, $id, $this->content_model->prefix);
                        if (IS_AJAX || IS_API_AUTH) {
                            exit(dr_json(1, fc_lang('发布成功，马上返回列表'), dr_member_url(APP_DIR.'/home/index'), $id));
                        }
                        $this->template->assign(array(
        ...
        if (isset($_GET['title']) && $_GET['title']) {
            $data['title'] = dr_safe_replace($_GET['title']);
        }
    ...
    public function edit() {
        ...
        // 数据是否存在
        if (!$data) {
            $this->member_msg(fc_lang('对不起，数据被删除或者查询不存在').'('.$id.')');
        }
    ...
    public function favorite() {
        ...
        $table = $this->link->dbprefix(SITE_ID.'_'.APP_DIR.'_favorite');
        ...
        if (IS_POST) {
            ...
            foreach ($ids as $id) {
                $id = intval($id);
                $row = $this->link->where('id', $id)->where('uid', $this->uid)->get($table)->row_array();
                if ($row) {
                    $this->link->where('id', $id)->delete($table);
                    // 更新收藏数量
                    if ($row['eid']) {
                        $c = $this->link->where('eid', (int)$row['eid'])->count_all_results($table);
                        $this->link->where('id', (int)$row['eid'])->set('favorites', $c)->update(SITE_ID.'_'.APP_DIR.'_extend');
                    } else {
                        $c = $this->link->where('cid', (int)$row['cid'])->count_all_results($table);
                        $this->link->where('id', (int)$row['cid'])->set('favorites', $c)->update(SITE_ID.'_'.APP_DIR);
                    }
                }
            }
            exit(dr_json(1, fc_lang('操作成功，正在刷新...')));
        ...
        if (!$total) {
            ...
            $data = $this->link->get($table)->row_array();
            $total = (int)$data['total'];
        }

        if ($total) {
            $page = max((int)$this->input->get('page'), 1);
            $this->link->where('uid', $this->uid)->order_by('inputtime desc');
            $list = $this->link->limit($this->pagesize, $this->pagesize * ($page - 1))->get($table)->result_array();
        }
    ...
    public function buy() {
        ...
        $table = $this->link->dbprefix(SITE_ID.'_'.APP_DIR.'_buy');
        ...
        if (IS_POST) {
            ...
            foreach ($ids as $id) {
                $this->link->where('id', intval($id))->where('uid', $this->uid)->delete($table);
            }
            exit(dr_json(1, fc_lang('操作成功，正在刷新...')));
        }
    ...
    public function draft() {
        ...
        if ($page == 1) {
            $this->template->assign(array(
                'list' => $data,
                'moreurl' => 'index.php?s=member&mod='.APP_DIR.'&c=home&m=draft'
            ));
            ...
            $this->template->display('module_draft_data.html');
        }
    }
