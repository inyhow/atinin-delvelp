...
class D_Admin_Form extends M_Controller {
    ...
    public function __construct() {
		...
		$this->cdata = $this->link->where('id', $this->cid)->get(SITE_ID.'_'.APP_DIR)->row_array();
        ...
        if ($this->cid) {
			$my = array(
				lang('返回') => array($this->_get_back_url(APP_DIR.'/home/index'), 'mail-reply'),
			...
			if ($this->router->method == 'show') {
				$my[fc_lang('查看')] = array(APP_DIR.'/admin/'.$this->router->class.'/show/cid/'.$this->cid.'/id/'.intval($_GET['id']), 'search');
			}
        } else {
			$my = array(
			...
			if ($this->router->method == 'show') {
				$my[fc_lang('查看')] = array(APP_DIR.'/admin/'.$this->router->class.'/show/id/'.intval($_GET['id']), 'search');
			}
        }
        ...
        if (IS_ADMIN && $this->admin['adminid'] > 1) {
            ...
            if ($category) {
                ...
                unset($category);
                if ($catid) {
                    $data = $this->link->select('id')->where_in('catid', $catid)->get(SITE_ID.'_'.APP_DIR.'_index')->result_array();
                    if ($data) {
                        foreach ($data as $t) {
                            $this->ids[] = (int)$t['id'];
                        }
    ...
    public function index() {
		...
		if ($this->input->post('action') == 'del') {
			...
			foreach ($ids as $id) {
                ...
                if ($row) {
                    $this->link->where('id', $row['cid'])->set($this->fid.'_total', $this->fid.'_total - 1', FALSE)->update(SITE_ID.'_'.APP_DIR);
                    $this->link->where('id', $id)->delete($table);
                    $this->link->where('id', $id)->delete($table.'_data_'.(int)$row['tableid']);
    ...
    public function edit() {
        ...
        if ($this->ids && !in_array($data['cid'], $this->ids)) {
            $this->admin_msg(fc_lang('抱歉！您无权限操作(%s)', $data['id']));
        }
		...
		if (IS_POST) {
			...
			if (isset($data['error'])) {
                ...
                if ($post[0]) {
                    $this->link->where('id', (int)$id)->update($table.'_data_'.intval($data['tableid']), $post[0]);
                } else {
                    $post[0]['id'] = (int)$id;
                    $this->link->replace($table.'_data_'.intval($data['tableid']), $post[0]);
                }
				// 操作成功处理附件
				$this->attachment_handle($data['uid'], $table.'-'.$id, $this->field, $post);
                $this->system_log('删除站点【#'.SITE_ID.'】模块【'.APP_DIR.'】表单【'.$this->fid.'】内容【#'.$id.'】'); // 记录日志
