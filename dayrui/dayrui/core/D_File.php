...
class D_File extends M_Controller {
    ...
    protected function _init() {
        if (APP_DIR && APP_DIR != 'member' && APP_DIR != 'space') {
            $this->path = $this->router->method == 'mobile' || $this->input->get('ismb') ? FCPATH.'module/'.APP_DIR.'/mobiles/' : FCPATH.'module/'.APP_DIR.'/templates/';
			if ($this->router->class == 'theme') {
	...
	public function add() {
		...
		$error = $file = '';
		if (!is_dir($path)) {
            exit('<p style="padding:10px 20px 20px 20px">'.fc_lang('文件目录不存在').'</p>');
        }
		
		if (IS_POST) {
			$file = trim(str_replace(array('/', '\\'), '', $this->input->post('file')), '/');
			if (is_file($path.$file)) {
                exit(dr_json(0, fc_lang('文件或者目录已经存在了'), 'file'));
            }
			$ext = strrchr($file, '.');
			if ($ext) {
				// 创建文件
	...
	public function jie() {
        ...
        if (!is_dir($path)) {
            $this->admin_msg(fc_lang('文件目录不存在'));
        }
	...
	public function del() {
        ...
        $file = trim(str_replace(array('../', '\\'), array('', '/'), $this->input->get('file')), '/');
		if (!$file) {
            exit(dr_json(0, fc_lang('文件或者目录格式不正确')));
        }
    ...
    public function store() {
        $this->template->assign(array(
            'url' => 'http://store.dayrui.com/index.php?c=category&id=2&action=application&param='.dr_base64_encode(dr_array2string(array(
                'site' => SITE_URL,
                'name' => SITE_NAME,
                'admin' => SELF,
