...
class D_Home_Form extends M_Controller {
    ...
    public function __construct() {
        ...
        if (!$this->cdata) {
            $this->msg(fc_lang('内容(id#%s)不存在', $this->cid));
        }
        ...
        // 禁用权限
        if ($rule['disabled']) {
            $this->msg(fc_lang('当前会员组无权限操作'));
        }
        // 每日发布数量检查
        if ($rule['postnum']) {
            $total = $this->link->where('uid', $this->uid)->where('DATEDIFF(from_unixtime(inputtime),now())=0')->count_all_results($this->table);
            if ($total >= $rule['postnum']) {
                $this->msg(fc_lang('每日发布数量不得超过%s个', $rule['postnum']));
            }
        }
        // 投稿总数检查
        if ($rule['postcount']) {
            ...
            if ($total >= $rule['postcount']) {
                $this->msg(fc_lang('发布总数不得超过%s个', $rule['postcount']));
            }
        }
        // 金币检查
        if ($rule['score'] + $this->member['score'] < 0) {
            $this->msg(fc_lang(SITE_SCORE.'不足！本次需要%s'.SITE_SCORE.'，当前余额%s'.SITE_SCORE, abs($rule['score']), $this->member['score']));
        }
    ...
    protected function _add($data) {
        ...
        if (($id = $this->link->insert_id())
            ...
            if ($this->form['setting']['email']) {
                $this->sendmail_queue($user['email'], $title, dr_lang('《%s》有新的%s', $this->cdata['title'], $this->form['name'], $murl, $murl));
            }
            // 短信提醒
