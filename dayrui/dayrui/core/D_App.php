...
class D_App extends D_Common {
    ...
    public function __construct() {
        ...
        // 应用不存在或者被禁用
        if (!IS_ADMIN && !$this->app) {
            $this->msg(fc_lang('应用尚未安装或者被禁用了'));
        }
        // 定义应用模板路径
    ...
    public function _admin_config() {
        ...
        if (!$this->is_auth('admin/application/config')) {
            $this->admin_msg(fc_lang('抱歉！您无权限操作(%s)', 'application/config'));
        }
            ...
            if ($this->application_model->edit($data['id'], array(
                ...
                if ($del) {
                    foreach ($del as $dir => $value) {
                        if ($value) {
                            $this->_delete_for_module($dir); // 删除该模块时
                        }
                    }
                }
        ...
        $mod = array();
        $local = @array_diff(dr_dir_map(FCPATH, 1), array('app', 'cache', 'config', 'dayrui', 'member', 'space')); // 搜索本地模块
        if ($local) {
            foreach ($local as $dir) {
                if (is_file(FCPATH.$dir.'/config/module.php')) {
                    $mod[$dir] = require FCPATH.$dir.'/config/module.php';
                }
            }
        }
    ...
    public function _admin_install() {
        ...
        if (!$this->is_auth('admin/application/install')) {
            $this->admin_msg(fc_lang('抱歉！您无权限操作(%s)', 'application/install'));
        }

        if ($this->db->where('dirname', APP_DIR)->count_all_results('application')) {
            $this->admin_msg(fc_lang('应用【%s】已经存在，安装失败', APP_DIR));
        }
        // 插入应用数据库
        ...
        // 安装菜单
        if (is_file(FCPATH.'app/'.APP_DIR.'/config/menu.php') && (D_MARK || $this->ci->mark)) {
    ...
    public function _admin_uninstall() {
        ...
        if (!$this->is_auth('admin/application/uninstall')) {
            $this->admin_msg(fc_lang('抱歉！您无权限操作(%s)', 'application/uninstall'));
        }
