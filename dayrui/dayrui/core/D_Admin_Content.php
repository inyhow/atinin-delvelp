...
class D_Admin_Content extends M_Controller {
    ...
    public function keyword() {
        ...
        if (IS_POST) {
            ...
            // 统计数量
            if ($keyword) {
                $total = $query->where('keywords=""')->count_all_results($this->content_model->prefix);
            } else {
        ...
        if ($this->input->get('todo')) {
            ...
            if ($cache) {
                ...
                $catid = 0;
                $keyword = 1;
                $total = $this->link->where('keywords=""')->count_all_results($table);
            }
            ...
            foreach ($data as $t) {
				...
				if ($info) {
					...
					$update = @implode(',', $kws);
					if ($update) {
						$this->link->where('id='.$t['id'])->update($table, array(
							'keywords' => $update
						));
    ...
    public function index() {
        ...
        if ($this->input->get('todo')) {
            ...
            $data = $this->link->select('id,tableid')->limit($psize, $psize * ($page - 1))->order_by('id DESC')->get($table)->result_array();
            foreach ($data as $t) {
                $row = $this->link->select('content')->where('id', $t['id'])->get($table.'_data_'.$t['tableid'])->row_array();
                if ($row) {
                    $thumb = 0;
                    ...
                    if ($thumb) {
                        ...
                        if (IS_SHARE) {
                            $this->link->where('id', $t['id'])->update($this->content_model->share_prefix, array('thumb' => $thumb));
                        }
                    }
                }
            }
    ...
    public function url() {
        ...
        if ($this->input->get('todo')) {
            ...
            if ($cache) {
                ...
                $catid = 0;
                $total = $this->link->count_all_results($this->content_model->prefix.($extend ? '_extend' : '').'_index');
            }
            $tpage = ceil($total / $psize); // 总页数
            if ($page > $tpage) {
                // 更新完成删除缓存
            ...
            if ($extend) {
                ...
                foreach ($data as $e) {
                    ...
                    $this->link->where('id',(int)$e['id'])->update($table, array(
                        'url' => $url
                    ));
                }
            } else {
                ...
                foreach ($data as $t) {
                    ...
                    if (IS_SHARE) {
                        $this->link->where('id', $t['id'])->update($this->content_model->share_prefix, array('url' => $url));
                    }
                }
            }
