...
class D_Admin_Extend extends M_Controller {
    ...
    public function __construct() {
		...
		if ($this->admin['adminid'] > 1 && !$this->catrule['show']) {
			if (IS_AJAX) {
				exit(dr_json(0, fc_lang('当前管理组角色您无权限操作此模块内容')));
			}
		...
		if (IS_POST && !$this->input->post('search')) {
			...
			switch($this->input->post('action')) {
				...
				case 'order':
					...
					foreach ($ids as $id) {
						$this->link->where('id', $id)->update($this->content_model->prefix.'_extend', $_data[$id]);
					}
			...
			if (isset($data['error'])) {
				...
				if ($id = $this->content_model->add_extend($data)) {
					...
					if ($rule['extend_experience'] + $member['experience'] >= 0) {
						$this->member_model->update_score(0, $this->content['uid'], $rule['extend_experience'], $mark, fc_lang('发布内容'), 1);
					}
					// 金币处理
					if ($rule['extend_score'] + $member['score'] >= 0) {
						$this->member_model->update_score(1, $this->content['uid'], $rule['extend_score'], $mark, fc_lang('发布内容'), 1);
					}
					// 操作成功处理附件
