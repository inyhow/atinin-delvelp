...
class C_Model extends CI_Model {
    ...
    public function limit_page($param, $page, $total) {
        ...
        if (isset($_param['flag'])) {
            ...
            if ($ids) {
                ...
                $data = $this->link->where_in('id', $in)->order_by($_order)->get($this->prefix)->result_array();
            }
        } else {
    ...
    public function index($data) {
        ...
        if (defined('IS_SHARE') && IS_SHARE) {
            // 共享模块
            $this->link->insert($this->share_prefix.'_index', array(
                'uid' => $data[1]['uid'],
                'mid' => $this->mdir,
                'catid' => $data[1]['catid'],
    ...
    public function add($data, $syncatid = 0) {
        ...
        // 兼容性处理
        if (SYS_UPDATE && !$field['link_id']) {
            // 创建同步字
            $this->link->query("ALTER TABLE  `{$this->prefix}` ADD  `link_id` INT( 10 ) DEFAULT '0' COMMENT  '同步id'");
        }
        ...
        if ($data[1]['status'] >= 9) {
            ...
            if (!$this->link->query("SHOW TABLES LIKE '".$this->prefix.'_data_'.$data[1]['tableid']."'")->row_array() && (D_MARK || $this->ci->mark)) {
                // 附表不存在时创建附表
            ...
            if ($syncatid) {
                ...
                if ($syn) {
                    // 更新主表状态主表
                    $this->link->where('id', $id)->update($this->prefix, array('link_id' => -1));
                    foreach ($syn as $cid) {
                        if ($cid && $cid != $data[1]['catid']) {
                            ...
                            $this->link->replace($this->prefix, $new[1]); // 主表
                            // 同步共享表
                            if (defined('IS_SHARE') && IS_SHARE) {
                                $this->update_share($data[1]);
                            }
    ...
    public function edit($_data, $data, $oid = 0) {
        ...
        if ($_data['link_id'] != 0) {
            // 更新当前同步数据的栏目
            $this->link->where('id', $oid)->update($this->prefix.'_index', array(
                'catid' => $data[1]['catid'],
            ));
            $this->link->where('id', $oid)->update($this->prefix, array(
                'catid' => $data[1]['catid'],
            ));
        ...
        if ($data[1]['status'] >= 9) {
            ...
            if ($_uid != $data[1]['uid']) {
                $this->link->where('id', intval($_data['id']))->update($this->prefix.'_index', array(
                    'uid' => $_uid,
                ));
            }
            // 分析栏目字段数据
            ...
            $this->link->where('id', $_data['id'])->update($this->prefix.'_index', array(
                'uid' => $data[1]['uid'],
                'catid' => $data[1]['catid'],
                'status' => $data[1]['status']
            ));
            ...
            if ($_data['link_id'] != 0) {
                ...
                $this->link->where('link_id', $_data['id'])->update($this->prefix, $syn);
                // 同步共享表
                if (defined('IS_SHARE') && IS_SHARE) {
                    $this->link->where('link_id', $_data['id'])->update($this->share_prefix, $syn);
                }
            }
            ...
            // 更新主表
            $this->link->where('id', (int)$_data['id'])->update($this->prefix, array(
                'status' => $data[1]['status']
            ));
            // 更新索引表
            $this->link->where('id', intval($_data['id']))->update($this->prefix.'_index', array(
            ...
            // 同步共享表
            if (defined('IS_SHARE') && IS_SHARE) {
                $this->link->where('id', intval($_data['id']))->update($this->share_prefix.'_index', array(
                    'status' => $data[1]['status']
                ));
                $this->link->where('id', intval($_data['id']))->update($this->share_prefix, array(
                    'status' => $data[1]['status']
                ));
            }
            // 更新审核表
    ...
    private function extend_index($data) {
        ...
        if (defined('IS_SHARE') && IS_SHARE) {
            // 共享模块
            $this->link->insert($this->share_prefix.'_extend_index', array(
                'uid' => $data[1]['uid'],
                'mid' => $this->mdir,
                'cid' => $data[1]['cid'],
                'catid' => $data[1]['catid'],
    ...
    public function add_extend($data) {
        ...
        if ($data[1]['status'] >= 9) {
            ...
            // 更新内容表时间
            $this->link->where('id', (int)$data[1]['cid'])->update($this->prefix, array('updatetime' => SYS_TIME));
            // 审核表
            $this->link->where('id', (int)$data[1]['id'])->delete($this->prefix.'_extend_verify');
        } else {
            $data[0]['is_new'] = 1; // 表示新文章
    ...
    public function edit_extend($_data, $data) {
        ...
        if ($data[1]['status'] >= 9) {
            ...
            // 更新索引表
            $this->link->where('id', $_data['id'])->update($this->prefix.'_extend_index', array(
                'uid' => $data[1]['uid'],
                'catid' => $data[1]['catid'],
                'status' => $data[1]['status']
            ));
            ...
            // 副表
            $this->link->replace($this->prefix.'_extend_data_'.$data[1]['tableid'], $data[0]);
            // 审核表
            if ($_data['status'] < 9) {
                $this->link->where('id', $_data['id'])->delete($this->prefix.'_extend_verify');
            }
            // 同步共享表
            if (defined('IS_SHARE') && IS_SHARE) {
                $this->update_share($data[1], 1);
                // 更新索引表
                $this->link->where('id', $_data['id'])->update($this->share_prefix.'_extend_index', array(
                    'uid' => $data[1]['uid'],
                    'catid' => $data[1]['catid'],
                    'status' => $data[1]['status']
                ));
            ...
            // 更新内容表时间
            $this->link->where('id', $data[1]['cid'])->update($this->prefix, array('updatetime' => SYS_TIME));
            // 分享信息
            $data[1]['title'] = trim($this->content['title']).' - '.$data[1]['name'];
            $this->sns_share($data, 1); // 分享信息
        } else {
            ...
            $content['title'] = $this->content['title'];
            // 更新主表
            $this->link->where('id', (int)$_data['id'])->update($this->prefix.'_extend', array(
                'status' => $data[1]['status']
            ));
            // 更新索引表
            $this->link->where('id', intval($_data['id']))->update($this->prefix.'_extend_index', array(
                'status' => $data[1]['status']
            ));
            // 同步共享表
            if (defined('IS_SHARE') && IS_SHARE) {
                // 更新主表
                $this->link->where('id', (int)$_data['id'])->update($this->share_prefix.'_extend', array(
                    'status' => $data[1]['status']
                ));
                // 更新索引表
                $this->link->where('id', intval($_data['id']))->update($this->share_prefix.'_extend_index', array(
                    'status' => $data[1]['status']
                ));
            }
            // 更新审核表
    ...
    public function get_extend($id) {
        ...
        // 主表
        $data1 = $this->link->where('id', $id)->limit(1)->get($this->prefix.'_extend')->row_array();
        // 副表
        $data2 = $this->link->where('id', $id)->limit(1)->get($this->prefix.'_extend_data_'.(int)$data1['tableid'])->row_array();
    ...
    public function get($id) {
        ...
        // 主表
        $data1 = $this->link->where('id', $id)->limit(1)->get($this->prefix)->row_array();
        if (!$data1) {
            return NULL;
        }

        // 副表
        $data2 = $this->link->where('id', $id)->limit(1)->get($this->prefix.'_data_'.$data1['tableid'])->row_array();

        // 栏目附加数据
        $data3 = $this->link->where('id', $id)->limit(1)->get($this->prefix.'_category_data')->row_array();
        if ($data3) {
            // 栏目附加数据副表
            $data4 = $this->link->where('id', $id)->limit(1)->get($this->prefix.'_category_data_'.$data1['tableid'])->row_array();
        }
    ...
    public function get_verify($id) {
        ...
        // 主表
        $data = $this->link->where('id', $id)->limit(1)->get($this->prefix.'_verify')->row_array();
        if (!$data) {
            return NULL;
        }
    ...
    public function get_extend_verify($id) {
        ...
        // 主表
        $data = $this->link->where('id', $id)->limit(1)->get($this->prefix.'_extend_verify')->row_array();
        if (!$data) {
            return NULL;
        }
    ...
    public function sns_share($body, $is_extend = 0) {
        ...
        if ($syn) {
            if (isset($data[$syn]) && $data[$syn]) {
                $title = $data[$syn];
            } else {
                log_message('error', '无法执行自定义同步内容：同步字段（'.$syn.'）内容为空');
            }
        }
        ...
        // 百度ping
        if ($this->ci->get_cache('module-'.SITE_ID.'-'.$this->mdir, 'setting', 'bdping')) {
            $this->cron_model->add(5, array(
                'url' => $url,
                'site' => SITE_URL,
    ...
    public function delete_for_id($id, $tableid) {
        ...
        // 删除审核表
        $this->link->where('id', $id)->delete($this->prefix.'_verify');
        // 删除索引表
        $this->link->where('id', $id)->delete($this->prefix.'_index');
        // 删除附表表
        $this->link->where('id', $id)->delete($this->prefix.'_data_'.(int)$tableid);
        // 删除标记表
        $this->link->where('id', $id)->delete($this->prefix.'_flag');
        // 删除统计
        $this->link->where('id', $id)->delete($this->prefix.'_hits');
        // 删除主表
        $this->link->where('id', $id)->delete($this->prefix);
        // 当存在栏目附加表时
        if ($this->link->query("SHOW TABLES LIKE '".$this->prefix.'_category_data_'.(int)$tableid."'")->row_array()) {
            // 删除栏目附加表
            $this->link->where('id', $id)->delete($this->prefix.'_category_data');
            // 删除栏目附加表
            $this->link->where('id', $id)->delete($this->prefix.'_category_data_'.(int)$tableid);
        }
        // 删除收藏表
        $this->link->where('id', $id)->delete($this->prefix.'_favorite');
        $this->link->where('id', $id)->delete($this->prefix.'_buy');
        // 删除应用的相关表
        ...
        // 删除文件
        if ($html = $this->link->select('filepath,id')->where('rid', $id)->where('type', 1)->get($this->prefix.'_html')->result_array()) {
        ...
        if ($this->ci->get_cache('module-'.SITE_ID.'-'.$this->mdir, 'extend')) {
            $data = $this->link->select('tableid,id')->where('cid', $id)->get($this->prefix.'_extend')->result_array();
            if ($data) {
                // 逐一删除内容
                foreach ($data as $t) {
        ...
        // 删除草稿
        if ($data = $this->link->where('cid', $id)->get($this->prefix.'_draft')->result_array()) {
            foreach ($data as $t) {
                $this->link->where('id', $t['id'])->delete($this->prefix.'_draft');
        ...
        if ($form) {
            foreach ($form as $f) {
                ...
                $data = $this->link->where('cid', $id)->get($table)->result_array();
                if ($data) {
                    foreach ($data as $t) {
        ...
        // 删除评论
        $index = $this->link->where('cid', $id)->get($this->prefix.'_comment_index')->row_array();
        if ($index) {
            $this->link->where('cid', $id)->delete($this->prefix.'_comment_my');
            $this->link->where('cid', $id)->delete($this->prefix.'_comment_index');
    ...
    public function delete_extend_for_ids($ids) {
        ...
        $data = $this->link->select('tableid,id,cid')->where_in('id', $ids)->get($this->prefix.'_extend')->result_array();
        if (!$data) {
            return NULL;
        }
    ...
    public function delete_extend_for_id($id, $cid, $tableid) {
        ...
        // 删除索引表
        $this->link->where('id', $id)->delete($this->prefix.'_extend_index');
        $this->link->where('id', $id)->delete($this->prefix.'_extend');
        $this->link->where('id', $id)->delete($this->prefix.'_extend_verify');

        // 删除附表
        $this->link->where('id', $id)->delete($this->prefix.'_extend_data_'.(int)$tableid);
        ...
        // 删除草稿
        if ($data = $this->link->where('cid', $cid)->where('eid', $id)->get($this->prefix.'_draft')->result_array()) {
            foreach ($data as $t) {
                $this->link->where('id', $t['id'])->delete($this->prefix.'_draft');
                $this->attachment_model->delete_for_table($this->prefix.'_draft-'.$t['id']);
            }
        ...
        // 删除评论
        $index = $this->link->where('cid', $id)->get($this->prefix.'_extend_comment_index')->row_array();
        if ($index) {
            $this->link->where('cid', $id)->delete($this->prefix.'_extend_comment_my');
            $this->link->where('cid', $id)->delete($this->prefix.'_extend_comment_index');
    ...
    public function del_extend_verify($id) {
        ...
        if (!$this->link->where('id', $id)->count_all_results($this->prefix.'_extend')) {
            ...
            // 主表有数据时 恢复为通过状态
            $this->link->where('id', $id)->update($this->prefix.'_extend_index', array('status' => 9));
            $this->link->where('id', $id)->update($this->prefix.'_extend', array('status' => 9));
            return FALSE;
        }
    }
    ...
    public function del_verify($id) {
        ...
        if (!$this->link->where('id', $id)->count_all_results($this->prefix)) {
            ...
            // 主表有数据时 恢复为通过状态
            $this->link->where('id', $id)->update($this->prefix.'_index', array('status' => 9));
            $this->link->where('id', $id)->update($this->prefix, array('status' => 9));
            return FALSE;
        }
    }
    ...
    public function flag($ids, $flag) {
        ...
        $data = $this->link->where_in('id', $ids)->select('catid,id')->get($this->prefix)->result_array();
        if (!$data) {
            return NULL;
        }
        ...
        foreach ($data as $t) {
            if ($flag > 0) {
                ...
                // 取消推荐位
                $this->link->where('id', $t['id'])->where('flag', abs($flag))->delete($this->prefix.'_flag');
                $i ++;
            }
        }
    ...
    public function move($id, $catid) {
        ...
        $this->link->where_in('id', $id)->update($this->prefix, array('catid' => $catid));
        $this->link->where_in('id', $id)->update($this->prefix.'_index', array('catid' => $catid));

        if ($this->ci->get_cache('module-'.SITE_ID.'-'.$this->mdir, 'extend')) {
            $this->link->where_in('cid', $id)->update($this->prefix.'_extend', array('catid' => $catid));
            $this->link->where_in('cid', $id)->update($this->prefix.'_extend_index', array('catid' => $catid));
        }
    ...
    public function extend_move($ids, $type) {
        ...
        $this->link->where_in('id', $ids)->update($this->prefix.'_extend', array('mytype' => $type));
    ...
    public function set_html($type, $uid, $cid, $rid, $catid, $filepath) {
        ...
        if ($type != 3
            ...
            $this->link->where('rid', $rid)->where('type', $type)->update($table, array(
                'cid' => $cid,
                'uid' => $uid,
                'type' => $type,
                'catid' => $catid,
    ...
    public function save_draft($id, $data, $is_et = 0) {
        ...
        // 判断草稿是否存在，不存在就插入
        if ($id && $this->link->where('id', $id)->count_all_results($this->prefix.'_draft')) {
            $this->link->where('id', $id)->update($this->prefix.'_draft', $data);
        } else {
            $this->link->insert($this->prefix.'_draft', $data);
            $id = $this->link->insert_id();
        }
    ...
    public function delete_draft($id, $where) {
        ...
        if ($this->link->where('id', $id)->where('uid', $this->uid)->where($where)->get($this->prefix.'_draft')->row_array()) {
            $this->link->where('id', $id)->delete($this->prefix.'_draft');
            return TRUE;
        }
    ...
    public function get_draft($id) {
        ...
        $data = $this->link->where('id', $id)->where('uid', $this->uid)->get($this->prefix.'_draft')->row_array();
        if (!$data) {
            return NULL;
        }
    ...
    public function get_draft_list($where) {
        ...
        $rt = array();
        $data = $this->link->where('uid', $this->uid)->where($where)->order_by('inputtime desc')->get($this->prefix.'_draft')->result_array();
        if ($data) {
            foreach ($data as $t) {
    ...
    public function update_share($data, $extend = 0) {
        ...
        $this->link->replace($table, $update);
    }
