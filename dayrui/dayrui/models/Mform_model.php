...
class Mform_model extends CI_Model {
	...
	public function add($dir, $data) {
		...
		if ($id = $this->db->insert_id()) {
            ...
            foreach ($module as $sid => $mod) {
                ...
                $this->site[$sid]->query(str_replace('{tablename}', $table, $sql1));
                // 附表
                $this->site[$sid]->query("DROP TABLE IF EXISTS `".$table."_data_0`");
                $this->site[$sid]->query(str_replace('{tablename}', $table.'_data_0', $sql2));
                // 模块表创建统计字段
	...
	public function get($id, $fid, $dir = '') {
		...
		$data2 = $this->link->where('id', $id)->get($table.'_data_'.intval($data['tableid']))->row_array();
