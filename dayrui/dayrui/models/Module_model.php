	...
	public function export_form($dir) {
		...
		if ($form) {
			...
			foreach ($form as $t) {
                ...
                // 主表
				$sql = $this->site[SITE_ID]->query("SHOW CREATE TABLE `".$table."`")->row_array();
				$sql = str_replace(array($sql['Table'], 'CREATE TABLE'), array('{tablename}', 'CREATE TABLE IF NOT EXISTS'), $sql['Create Table']);
				// 附表
                $sql2 = $this->site[SITE_ID]->query("SHOW CREATE TABLE `".$table."_data_0`")->row_array();
				$sql2 = str_replace(array($sql['Table'], 'CREATE TABLE'), array('{tablename}', 'CREATE TABLE IF NOT EXISTS'), $sql2['Create Table']);
				// 模块表单的自定义字段
		...
		if ($dirname == 'share') {
			...
			foreach ($this->site_info as $siteid => $t) {
				...
				if (isset($data['site'][$siteid]['use']) && $data['site'][$siteid]['use']) {
					...
					if ($config['nodb'] == 0) {
						// 模块的栏目分类
						$cdir = $data['share'] ? 'share' : ($config['category'] ? $config['category'] : $dirname); // 判断关联栏目
						$category = $this->site[$siteid]
										->order_by('displayorder ASC, id ASC')
										->get($siteid.'_'.$cdir.'_category')
										->result_array();
						if ($category) {
							$CAT = $CAT_DIR = $fenzhan = $level = array();
							if (function_exists('dr_fenzhan_data')) {
							...
							foreach ($category as $c) {
								if ($update == 1) {
									if (!$c['child'] || $data['setting']['pcatpost']) {
										$c['total'] = $this->site[$siteid]->where('status', 9)->where('catid', $c['id'])->count_all_results($siteid.'_'.$cdir.'_index');
									} else {
										$c['total'] = 0;
									}
