...
class Page_model extends CI_Model {
    ...
    public function get_data() {
        ...
        $data = array();
        $_data = $this->link->where('module', APP_DIR)->order_by('displayorder ASC,id ASC')->get($this->tablename)->result_array();
        if (!$_data) {
            return $data;
        }
	...
	private function dirname_exitsts($dir, $id = 0) {
		...
		return $dir ? $this->link->where('dirname', $dir)->where('module', APP_DIR)->where('id<>', $id)->count_all_results($this->tablename) : 1;
	}
	...
	public function cache($siteid = SITE_ID) {
        ...
        $this->tablename = $this->link->dbprefix($siteid.'_page');
		$data = $this->link->order_by('displayorder ASC,id ASC')->get($siteid.'_page')->result_array();
		if ($data) {
            $cache = array();
            foreach ($data as $t) {
