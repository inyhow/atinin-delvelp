...
class Comment_model extends CI_Model {
    ...
    public function verify($table, $id) {
        ...
        if ($markrule && $row['uid']) {
            ...
            if ($permission['experience']) {
                // 增加经验值
                $this->member_model->update_score(0, $uid, abs($permission['experience']), '', '评论');
            }
            if ($permission['score']) {
                // 增加金币
                $this->member_model->update_score(1, $uid, $permission['score'], '', '评论');
            }
            // 我的评论
        ...
        // 回复评论时，将主题设置为存在回复状态
        if ($row['reply']) {
            $this->mydb->where('id', $row['reply'])->update($table, array(
                'in_reply' => 1,
            ));
        }
    ...
    public function post($uid, $data, $my = array()) {
        ...
        if ($rid && $row = $this->mydb->where('id', $rid)->get($table)->row_array()) {
            ...
            // 提醒被回复者
            if (!$data['verify']) {
                $this->member_model->add_notice($row['uid'], 2, fc_lang('您的评论被人回复，<a href="%s" target="_blank">查看详情</a>', $data['url']));
            }
        } else {
            // 提醒作者被评论
            if (!$data['verify']) {
                $this->member_model->add_notice($data['uid'], 2, fc_lang('您有新的评论，<a href="%s" target="_blank">查看详情</a>', $data['url']));
            }
        }
        ...
        // 自定义字段入库
        if (isset($my[1]) && count($my[1])) {
            $insert = array_merge($insert, $my[1]);
        }
        ...
        // 回复评论时，将主题设置为存在回复状态
        if ($insert['reply']) {
            $this->mydb->where('id', $insert['reply'])->update($table, array(
                'in_reply' => 1,
            ));
        }
        ...
        // 更新点评数据
        if ($data['review']) {
            $this->update_review($table, $insert);
        }
