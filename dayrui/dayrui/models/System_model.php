...
class System_model extends CI_Model {
    ...
    public function __construct() {
		...
		$this->config = array(
			...
			'SITE_SCORE' => '金币名称',
			'SITE_MONEY' => '金钱名称',
			'SITE_CONVERT' => '金币兑换金钱的比例',
            'SITE_PHONE_MONEY' => '购买联系方式金币数',
	...
	public function save_config($system, $config, $action = '') {
		...
		foreach ($this->config as $i => $note) {
            // 处理逻辑值
            if (in_array($i, array('SYS_ATTACHMENT_DB', 'SYS_DEBUG', 'SYS_UPDATE', 'SYS_NEWS', 'SYS_LOG', 'SITE_ADMIN_CODE', 'SYS_MEMCACHE', 'SYS_CRON_QUEUE'))) {
	...
	public function cache() {
		...
		// 按站点查询
		if ($this->site_info) {
			foreach ($this->site_info as $sid => $x) {
				$db = $this->site[$sid];
				if ($db !== $this->db && $_table = $db->query("SHOW TABLE STATUS FROM {$db->database}")->result_array() && (D_MARK || $this->ci->mark)) {
					foreach ($_table as $t) {
						if (strpos($t['Name'], $this->db->dbprefix) === 0) {
							$_field = $this->db->query('SHOW FULL COLUMNS FROM '.$t['Name'])->result_array();
							foreach ($_field as $c) {
            ...
            foreach ($data as $t) {
                ...
                if ($t['value'] && ($t['type'] == 2 || $t['type'] == 3)) {
                    ...
                    foreach ($t['value'] as $var => $val) {
                        if (strpos($var, 'share_') === 0) {
                            $t['value'][str_replace('share_', '', $var)] = $val;
                        }
                    }
                }
                $cache[$t['id']] = $t;
