...
class Category_share_model extends CI_Model {
    ...
    public function __construct() {
		...
		$this->prefix = $this->link->dbprefix(SITE_ID.'_share');
		$this->tablename = $this->link->dbprefix(SITE_ID.'_share_category');
    }
	...
	public function get_permission($id) {
		$data = $this->link->where('id', $id)->select('permission')->get($this->tablename)->row_array();
	...
	private function dirname_exitsts($dir, $id = 0) {
		return $dir ? $this->link->where('dirname', $dir)->where('id<>', $id)->count_all_results($this->tablename) : 1;
	}
