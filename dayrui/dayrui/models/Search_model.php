...
class Search_model extends CI_Model {
    ...
    public function clear($time) {
        ...
        $this->link->where('inputtime<', SYS_TIME - $time)->delete($this->indexname);
    }
    ...
    public function get($id, $page = 1) {
        ...
        $data = $this->link->where('id', $id)->get($this->tablename)->row_array();
        if (!$data) {
            return array();
        }
    ...
    public function set($get) {
        ...
        if (SITE_FID) {
            // 存在分站时
            $param['fid'] = isset($param['fid']) && $param['fid'] ? $param['fid'] : SITE_FID;
        }
        $order = $get['order'];
        ...
        if ($keyword != '') {
            ...
            foreach ($sfield as $t) {
                if ($t) {
                    $temp[] = '`'.$table.'`.`'.$t.'` LIKE "%'.$this->db->escape_str($keyword).'%"';
                }
            }
        ...
        foreach ($_order as $i => $t) {
            ...
            if (isset($mod_field[$a])) {
                $_order_by[$a] = $b ? $b : "DESC";
            }
        }
        if (!$_order_by) {
            $_order_by['updatetime'] = 'DESC';
        }
        unset($_order);
        ...
        if ($get['catid']) {
            ...
            if ($more) {
                $from.= ' LEFT JOIN `'.$table_more.'` ON `'.$table.'`.`id`=`'.$table_more.'`.`id`';
            }
        }
        // 筛选空值
		...
		if ($this->dir == 'mall' && function_exists('dr_mall_property') && $get) {
			...
			if ($property) {
				foreach ($get as $i => $v) {
					if (strpos($i, 'mall_') === 0) {
						$n = substr($i, 5);
						if (isset($property[$n]) && $property[$n] && strlen($v)) {
							$pwhere[] = '(pid='.intval($property[$n]['id']).' and value = "'.dr_safe_replace($v).'")';
						}
					}
				}
			}
			if ($pwhere) {
				// 组合查询
				$where.= ' AND `id` IN (select `cid` from '.$this->db->dbprefix(SITE_ID.'_'.$this->dir.'_property_search').' where '.implode(' and ', $pwhere).')';
			}
		}

        // 搜索商品有效期
        if (isset($mod_field['order_etime'])) {
            if (isset($get['oot']) && $get['oot']) {
                // 过期的
                $where.= ' AND `order_etime` BETWEEN 1 AND '.SYS_TIME;
            } else {
                // 正常的
                $where.= ' AND NOT (`order_etime` BETWEEN 1 AND '.SYS_TIME.')';
            }
        }
        ...
        if ($data) {
            // 入库索引表
            foreach ($data as $t) {
                $this->link->insert($this->indexname, array(
                    'id' => $id,
                    'cid' => $t['id'],
                    'inputtime' => SYS_TIME
                ));
