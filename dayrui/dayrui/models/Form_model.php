...
class Form_model extends CI_Model {
	...
	public function add($data) {
        ...
        } elseif ($this->link->where('table', $data['table'])->count_all_results($this->prefix)) {
	...
	public function editc($id, $table, $data) {
		...
		$this->link->where('id', (int)$id)->update($this->prefix.'_'.$table, $data);
	...
	public function new_editc($id, $table, $tableid, $data) {
		...
		$this->link->where('id', (int)$id)->update($this->prefix.'_'.$table, $data[1]);
        if ($data[0]) {
            $this->link->where('id', (int)$id)->update($this->prefix.'_'.$table.'_data_'.$tableid, $data[0]);
        } else {
            $data[0]['id'] = (int)$id;
            $this->link->replace($this->prefix.'_'.$table.'_data_'.$tableid, $data[0]);
        }
    ...
    public function get_data($id, $table) {
        ...
        $data = $this->link->where('id', $id)->get($table)->row_array();
        if (!$data) {
            return NULL;
        }

        $data2 = $this->link->where('id', $id)->get($table.'_data_'.(int)$data['tableid'])->row_array();
        if ($data2) {
