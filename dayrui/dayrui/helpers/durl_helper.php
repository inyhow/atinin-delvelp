...
function dr_mobile_show_url($dir, $id, $page = NULL) {
    if ($dir == 'store' && is_file(FCPATH.'module/store/config/.store')) {
        return SITE_URL.'index.php?s=store&id='.$id;
    }
    return SITE_URL.'index.php?s='.$dir.'&c=show&id='.$id.($page ? '&page='.$page : '');
}
...
function dr_mobile_extend_url($dir, $id, $page = NULL) {
    return SITE_URL.'index.php?s='.$dir.'&c=extend&id='.$id.($page ? '&page='.$page : '');
}
...
function dr_mobile_category_url($dir, $id, $page = NULL) {
    return SITE_URL.'index.php?s='.$dir.'&c=category&id='.$id.($page ? '&page='.$page : '');
}
...
function dr_mobile_page_url($dir, $id, $page = NULL) {
    return SITE_URL.'index.php?'.($dir ? 's='.$dir.'&' : '').'c=page&id='.$id.($page ? '&page='.$page : '');
}
...
function dr_share_search_url($params = NULL, $name = NULL, $value = NULL, $urlrule = NULL) {
    ...
    if (IS_MOBILE) {
        // 移动端
        return SITE_URL.'index.php?c=search&'.@http_build_query($params);
    } else {
        // PC端
        $ci	= &get_instance();
        $rule = $ci->get_cache('urlrule', (int)SITE_REWRITE, 'value');
...
function dr_search_url($params = NULL, $name = NULL, $value = NULL, $urlrule = NULL, $moddir = NULL) {
    ...
    // 当是分站且没有绑定域名自动加上参数
    if (SITE_FID && !isset($params['fid']) && defined('SITE_BRANCH_DOMAIN') && !SITE_BRANCH_DOMAIN) {
        $params['fid'] = SITE_FID;
    }
    ...
    if (IS_MOBILE) {
        // 移动端
        return SITE_URL.'index.php?s='.$dir.'&c=search&'.@http_build_query($params);
    } else {
        // PC端
        $ci	= &get_instance();
...
function dr_tag_url($module, $name, $page = 0) {
    ...
    if ($page) {
        $data['page'] = $page = is_numeric($page)  ? ($page > 1 ? $page : 0) : $page;
    }
...
function dr_linkagepos($code, $id, $symbol = ' > ', $url = NULL) {
    ...
    if (SITE_FID) {
        ...
        foreach ($pids as $pid) {
            if ($pid) {
                $name[] = $url ? "<a href=\"".str_replace('{linkage}', $cids[$pid], $url)."\">{$link[$cids[$pid]]['name']}</a>" : $link[$cids[$pid]]['name'];
            }
        }
        $name[] = $url ? "<a href=\"".str_replace('{linkage}', $id, $url)."\">{$data['name']}</a>" : $data['name'];
    }
...
function dr_catpos($catid, $symbol = ' > ', $url = TRUE, $html= '') {
	...
	foreach ($array as $id) {
		if ($id && $cat[$id]) {
            $murl = IS_PC ? $cat[$id]['url'] : dr_mobile_category_url(MOD_DIR, $id);
            if (SITE_FID) {
                /*
                if (IS_PC) {
    ...
    $murl = IS_PC ? $cat[$catid]['url'] : dr_mobile_category_url(MOD_DIR, $catid);
    if (SITE_FID) {
        /*
        if (IS_PC) {
            $murl = $cat[$id]['fenzhan'][SITE_FID];
...
function dr_page_catpos($id, $symbol = ' > ', $html = '') {
	...
	$page = MOD_DIR ? $page[MOD_DIR] : $page['index'];
	if (!isset($page[$id])) {
        return NULL;
    }
	...
	foreach ($array as $i) {
		if ($i && $page[$i]) {
            $murl = SITE_MOBILE === TRUE ? SITE_URL.(MOD_DIR ? MOD_DIR.'/' : '').'index.php?c=page&id='.$i : $page[$i]['url'];
			$name[] = $html ? str_replace(array('{url}', '{name}'), array($murl, $page[$i]['name']), $html) : "<a href=\"{$murl}\">{$page[$i]['name']}</a>";
		}
	}

    $murl = SITE_MOBILE === TRUE ? SITE_URL.(MOD_DIR ? MOD_DIR.'/' : '').'index.php?c=page&id='.$id : $page[$id]['url'];
...
function dr_space_catpos($uid, $catid, $symbol = ' > ', $url = TRUE, $html= '') {
	...
	foreach ($array as $id) {
		if ($id && $cat[$id]) {
			$name[] = $url ? ($html ? str_replace(array('{url}', '{name}'), array(dr_space_list_url($uid, $id), $cat[$id]['name']), $html) : "<a href=\"".dr_space_list_url($uid, $id)."\">{$cat[$id]['name']}</a>") : $cat[$id]['name'];
		}
	}
...
function dr_url($url, $query = array(), $self = SELF) {
    ...
    // 当是分站且没有绑定域名自动加上参数
    if (SITE_FID && !isset($query['fid']) && defined('SITE_BRANCH_DOMAIN') && !SITE_BRANCH_DOMAIN) {
        $query['fid'] = SITE_FID;
    }
