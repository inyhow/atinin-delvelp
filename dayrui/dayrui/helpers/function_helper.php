...
function dr_mform_total($cid, $mid, $module = APP_DIR, $cache = 10000) {
    ...
    if (!$data) {
        $data = $ci->site[SITE_ID]->where('cid', (int)$cid)->count_all_results(SITE_ID.'_'.$module.'_form_'.$mid);
...
function get_attachment($id) {
    ...
    if (in_array($info['fileext'], array('jpg', 'gif', 'png'))
        ...
        // 更新到数据表
        $ci->db->where('id', (int)$id)->update('attachment_'.(int)$data['tableid'], array(
...
function dr_thumb2($img, $width = 100, $height = 100, $autocut = 1) {
    ...
    // 当图片是附件id
    if (is_numeric($img)) {
        return SITE_URL."index.php?c=image&m=thumb2&p=$img-$width-$height-$autocut";
    } else {
        return dr_file($img);
    }
}
 ...
 * @return  url
 */
function dr_thumb($img, $width = NULL, $height = NULL, $water = NULL, $size = 0) {
    ...
    if (is_numeric($img)) { // 表示附件id
        ...
        $size or $size = (int)$size ;
        // 缓存文件
        $thumb_file = 'cache/thumb/'.md5("index.php?c=image&m=thumb&p=$id-$width-$height-$water-$size").'.jpg';
        // 存在缩略图时就输出图片
        ...
        return SITE_URL."index.php?c=image&m=thumb&p=$img-$width-$height-$water-$size";
    }
...
function dr_geetest($product = 'embed', $submit = '') {
    ...
    $add = '';
    $rid = rand(0, 99);
    $url = dr_http_prefix(strtolower($_SERVER['HTTP_HOST']).'/');
    if ($product == 'popup') {
...
var status = 0, result, apiFail;
            ...
            $.ajax({url : "'.$url.'index.php?c=api&m=geetest&rand="+Math.round(Math.random()*100),
                type : "get",
                dataType : "JSON",
