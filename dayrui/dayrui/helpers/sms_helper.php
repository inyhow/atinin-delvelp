...
function my_sms_send($phone, $content, $config = array()) {
	...
	//file_put_contents('testtt.txt',var_export($app,true)."\r\n",FILE_APPEND);exit;
	if (!$app['key']) {
...
*/
	$api_url = 'http://sh2.ipyy.com/sms.aspx';
	$action = 'send';

	$send_url = $api_url.'?action='.$action.'&userid=&account='.$app['key'].'&password='.$app['secret'].'&mobile='.$phone.'&content='.$content.'&sendTime=&extno=';
	
	$resp = file_get_contents($send_url);

	if (strpos($resp,'<returnstatus>Success</returnstatus>')!==false) {
