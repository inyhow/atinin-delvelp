...
class F_Files extends A_Field {
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
		...
		// 上传的URL
		$url = '&name='.$name.'&siteid='.SITE_ID.'&code='.str_replace('=', '', dr_authcode($cfg['option']['size'].'|'.$cfg['option']['ext'].'|'.$this->get_upload_path($cfg['option']['uploadpath']), 'ENCODE'));

		// 开始上传
		if ((IS_ADMIN && IS_PC) || (isset($cfg['option']['is_swfupload']) && $cfg['option']['is_swfupload'])) {
			//pc端采用Flash上传
			$url = '/index.php?s=member&c=api&m=upload'.$url;
			$file_value = '';
			...
			if ($value && isset($value['file'])) {
				foreach ($value['file'] as $id => $fileid) {
					...
					<tr>
						<td style="text-align:right;padding-right:5px;width:50px">
							<a href="javascript:;" title="'.fc_lang('删除').'" onclick="dr_remove_file(\''.$name.'\',\'999'.$id.'\')"><img align=\"absmiddle\" src="'.THEME_PATH.'admin/images/b_drop.png"></a>
							<a href="javascript:;" title="'.fc_lang('修改').'" onclick="dr_edit_file(\''.$url.'&count=1\',\''.$name.'\',\'999'.$id.'\')"><img align="absmiddle" src="'.THEME_PATH.'admin/images/b_edit.png"></a>
						</td>
						<td>
			...
			$str.= '	<legend>'.fc_lang('文件列表').'</legend>';
			...
			// 移动端样式
			$str = '';
			$url = '/index.php?s=member&c=api&m=new_ajax_upload'.$url;
			// 加载js
			...
			if ($value && isset($value['file'])) {
				foreach ($value['file'] as $id => $fileid) {
					...
					$title = $value['title'][$id];
					$my.= '
					<div class="row" style="margin-top:5px;" id="files_'.$name.'_999'.$id.'" list="999'.$id.'">
						<div class="col-md-7" style="padding-top:5px">
							...
							<input type="hidden" value="'.$fileid.'" name="data['.$name.'][file][]" id="fileid_'.$name.'_999'.$id.'" />
							<input type="text" class="form-control" value="'.$title.'" name="data['.$name.'][title][]"  placeholder="描述"> </div>
						</div>
			...
			if (!$disabled) {
				$str.= '
				<div id="drag-and-drop-zone-'.$name.'" class="btn blue uploader" style="cursor: pointer">
					<i class="fa fa-cloud-upload"></i> 单击上传（'.$cfg['option']['ext'].'）<input type="file" name="file">
				</div>';
			}
			$str.= '
          	<div id="dr_my_'.$name.'_list">'.$my.'</div>
    <script type="text/javascript">
      ...
      $("#drag-and-drop-zone-'.$name.'").dmUploader({
        ...
        onBeforeUpload: function(id){
          var vhtml = "";
		  var size = $("#dr_my_'.$name.'_list .row").size();
		  var mid = parseInt(size)+parseInt(id);
		  dr_'.$name.'_ids[id] = mid;
		  vhtml+= "<div class=\"row\" style=\"margin-top:5px;\" id=\"files_'.$name.'_"+mid+"\" list=\""+mid+"\">";
		  vhtml+= "<div class=\"col-md-7\" style=\"padding-top:5px\">";
		  vhtml+= "<div class=\"input-group\">";
		  vhtml+= "<span class=\"input-group-addon\" id=\"delete_'.$name.'_"+mid+"\"><i class=\"fa fa-cloud-upload\"></i></span>";
		  vhtml+= "<input type=\"hidden\" value=\"\" name=\"data['.$name.'][file][]\" id=\"fileid_'.$name.'_"+mid+"\" />";
		  vhtml+= "<input type=\"text\" class=\"form-control\" id=\"filename_'.$name.'_"+mid+"\" name=\"data['.$name.'][title][]\" placeholder=\"描述\" />";
		  vhtml+= "</div>";
		  vhtml+= "</div>";
		  vhtml+= "<div class=\"col-md-5\" id=\"loading_'.$name.'_"+mid+"\" style=\"padding-top:10px\">";
        ...
        onUploadSuccess: function(id, data){
		  var mid = dr_'.$name.'_ids[id];
		  if (data.code == 1) {
		    var vhtml = "<a href=\"javascript:;\" onclick=\"dr_show_file_info(\'"+data.id+"\')\">";
			vhtml+= "<img align=\"absmiddle\" src=\""+data.icon+"\"></a>";
		  	$("#loading_'.$name.'_"+mid).html(vhtml);
		  	$("#delete_'.$name.'_"+mid).html("<a href=\"javascript:;\" title=\"'.fc_lang('删除').'\" onclick=\"dr_remove_file(\''.$name.'\',\'"+mid+"\')\"><img align=\"absmiddle\" src=\"'.THEME_PATH.'admin/images/b_drop.png\"></a>");
		  	$("#fileid_'.$name.'_"+mid).val(data.id);
		  	$("#filename_'.$name.'_"+mid).val(data.name);
		  } else {
		    $("#files_'.$name.'_"+mid).remove();
		  	$("#dr_my_'.$name.'_list").append("<div style=\"margin-top:10px\" class=\"alert alert-danger\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\"></button><strong>"+data.msg+"</strong></div>");
		  }
