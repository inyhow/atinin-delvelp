...
class F_File extends A_Field {
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
        ...
        if (isset($cfg['option']['image']) && $cfg['option']['image']) {
			...
			if (IS_ADMIN) {
           		$str.= '<span class="rearrange-text" title="剪切图片" onclick="dr_edit_'.$name.'()" style="display: none;">剪切图片</span>';
			}
            $str.= '</div>';
            $str.= '</li>';
            ...
            $str.= $tips;
        } elseif ((IS_ADMIN && IS_PC) || (isset($cfg['option']['is_swfupload']) && $cfg['option']['is_swfupload'])) {
            // 当字段必填时，加入html5验证标签
            if (isset($cfg['validate']['required']) && $cfg['validate']['required'] == 1) {
                $attr.= ' required="required"';
            }
            // 上传的URL
            $url = '/index.php?s=member&c=api&m=upload&name='.$name.'&siteid='.SITE_ID.'&count=1&code='.str_replace('=', '', dr_authcode($cfg['option']['size'].'|'.$cfg['option']['ext'].'|'.$this->get_upload_path($cfg['option']['uploadpath']), 'ENCODE'));
            // 文件值
            $file = $info = '';
            if ($value) {
                $file = $value;
            ...
            // 上传按钮与表单值
            $tool = '<span id="file_html_'.$name.'"><input type="hidden" id="dr_'.$name.'" name="data['.$name.']" value="'.$file.'" '.$attr.' />
			<button type="button" style="cursor:pointer;" '.$disabled.'  class="btn blue btn-sm" onclick="dr_upload_file(\''.$name.'\', \''.$url.'\')"> <i class="fa fa-upload"></i> ' . fc_lang('上传文件') . '</button>
			';
            $str = $tool.$info.'</span>'.$tips;
        } else {
			// 会员中心时
			...
			if ($value) {
				...
				if ($data) {
					$my = '
					<button type="button" style="cursor:pointer;" class="btn green btn-sm" onclick="dr_show_file_info(\''.$data['id'].'\')"> <i class="fa fa-search"></i> '.$data['name'].'</button>
					<button type="button" style="cursor:pointer;"  class="btn red btn-sm" onclick="dr_delete_file2(\''.$name.'\')"> <i class="fa fa-trash"></i> ' . fc_lang('删除文件') . '</button>
					';
				} elseif (is_numeric($file) && !get_attachment($file)) {
      ...
      $("#drag-and-drop-zone-'.$name.'").dmUploader({
        ...
        onUploadSuccess: function(id, data){
		  if (data.code == 1) {
		  	...
		  	$("#dr_my_'.$name.'_list").html("<button type=\"button\" style=\"cursor:pointer;\" class=\"btn green btn-sm\" onclick=\"dr_show_file_info(\'"+data.id+"\')\"> <i class=\"fa fa-search\"></i> "+data.name+"</button> ");
