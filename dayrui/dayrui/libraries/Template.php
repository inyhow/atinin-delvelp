...
class Template {
    ...
    public $_dir; // 模板目录
    public $_mdir; // 优先选择的模板目录模式
    public $_mmdir; // 优先选择的模板目录模式
    public $_cache; // 模板缓存目录
    public $_root; // 默认前端项目模板目录
    public $_mroot; // 默认会员项目模板目录
    public $_aroot; // 默认后台项目模板目录
    public $_aaroot; // 其他分支系统默认后台项目模板目录
    public $_tpl_dir; // 模板目录名称
    ...
    public function __construct() {
        ...
        $this->_root = FCPATH.'dayrui/{templates}/'.SITE_TEMPLATE.'/'; // 默认主项目模板目录
        $this->_cache = WEBPATH.'cache/templates/'; // 模板缓存目录
        $this->_mroot = FCPATH.'module/member/{templates}/member/'.MEMBER_TEMPLATE.'/'; // 默认会员项目模板目录
        $this->_aroot = FCPATH.'dayrui/{templates}/admin/'; // 默认后台模板目录
        $this->_aaroot = FCPATH.'dayrui/templates/admin/'; // 其他分支系统默认后台模板目录
        ...
        if (IS_ADMIN) {
            ...
            $this->_mdir = FCPATH.'dayrui/{templates}/member/'.MEMBER_TEMPLATE.'/';
            if (APP_DIR != 'member') {
                $this->_mmdir = FCPATH.'dayrui/{templates}/member/'.MEMBER_TEMPLATE.'/'.APP_DIR.'/';
            }
        } elseif (APP_DIR && is_dir(FCPATH.'app/'.APP_DIR.'/')) {
            // 应用
            $this->_dir = APPPATH.'{templates}/'.$this->_tpl_dir.'/';
        } elseif (APP_DIR && is_dir(FCPATH.'module/'.APP_DIR)) {
            // 模块前端页面
            $this->_dir = APPPATH.'{templates}/'.$this->_tpl_dir.'/';
            $this->_mdir = FCPATH.'dayrui/{templates}/'.$this->_tpl_dir.'/'.APP_DIR.'/';
            $this->cron = SYS_CRON_QUEUE === 0 ? TRUE : FALSE; // 开启任务js
        } else {
            // 首页前端页面
            $this->_dir = FCPATH.'dayrui/{templates}/'.$this->_tpl_dir.'/';
            $this->cron = SYS_CRON_QUEUE === 0 ? TRUE : FALSE; // 开启任务js
        }
    }
    ...
    public function space($dir) {
        $this->_dir = $this->_mroot = WEBPATH.'statics/space/'.$dir.'/';
    }
    ...
    public function module($dir) {
        ...
        $this->_dir = FCPATH.'module/'.$module.'/{templates}/'.$dir.'/';
        $this->_mdir = FCPATH.'dayrui/{templates}/'.$dir.'/'.$module.'/';
        $this->_tpl_dir = $dir;
    }
     ...
     */
    public function template($path, $dir = MOD_DIR) {
        $this->_dir = FCPATH.'module/'.$dir.'/{templates}/'.$this->_tpl_dir.'/'.trim($path, '/').'/';
        $this->_mdir = FCPATH.'dayrui/{templates}/'.$this->_tpl_dir.'/'.$dir.'/'.trim($path, '/').'/';
    }
    ...
    public function get_file_name($file, $dir = NULL, $include = FALSE) {
        ...
        if (IS_ADMIN || $dir == 'admin') {
            ...
            if (@is_file($this->_dir.$file)) {
                ...
                // 当前项目目录模板不存在时调用主项目的
                return $this->_aroot.$file;
            } elseif (@is_file($this->_aaroot.$file)) {
                // 当前项目目录模板不存在时调用主项目的
                return $this->_aaroot.$file;
            }
            ...
            } elseif (APP_DIR != 'member' && is_file($this->_mmdir.$file)) {
                // 调用新风格下的主会员目录
                return $this->_mmdir.$file;
            ...
            if ($dir === '/' && is_file($this->_root.$file)) {
                // 强制主目录
                return $this->_root.$file;
            } else if ($this->_mdir && is_file($this->_mdir.$file)) {
                // 调用新风格下的模块目录
                return $this->_mdir.$file;
        ...
        if ($file == 'msg.html' && is_file(FCPATH.'dayrui/templates/default/msg.html')) {
    ...
    public function display($_name, $_dir = NULL) {
        ...
        // 模板选择
        $template = $this->mobile && MOD_DIR != 'weixin' ? 'mobiles' : 'templates';

        // 模板目录替换
        $this->_dir = str_replace('{templates}', $template, $this->_dir);
        $this->_mdir = str_replace('{templates}', $template, $this->_mdir);
        $this->_mmdir = str_replace('{templates}', $template, $this->_mmdir);
    ...
    public function _include($name, $dir = NULL) {
        ...
        if ($this->_include_file[$fname] > 50) {
            show_error('模板文件 ('.str_replace(WEBPATH, '/', $file).') 标签template引用文件目录结构错误', 500, '模板结构错误');
            exit;
        }
    ...
    public function _load($file) {
        ...
        if ($this->_include_file[$fname] > 50) {
            show_error('模板文件 ('.str_replace(WEBPATH, '/', $file).') 标签load引用文件目录结构错误', 500, '模板结构错误');
            exit;
        }
...
$.ajax({  
    ...
    public function code2php($code) {
        ...
        if (!is_file($this->_cache.$file)) {
            @file_put_contents($this->_cache.$file, str_replace('$this->', '$this->ci->template->', $this->handle_view_file($code)));
        }
        ...
        switch ($system['action']) {
            ...
            case 'linkage': // 联动菜单
                ...
                if (isset($param['pid'])) {
                    if (is_numeric($param['pid'])) {
                        ...
                        if (!$pid && is_numeric($param['pid']) && $this->ci->get_cache('linkage-'.$system['site'].'-'.$param['code'].'-id', $param['pid'])) {
                            $pid = intval($param['pid']);
                        }
                    }
                }
                ...
                if ($system['catid']) {
                    ...
                    if (strpos($system['catid'], ',') !== FALSE) {
                        ...
                        if ($temp) {
                            $catids = array();
                            foreach ($temp as $i) {
                                if ($module['category'][$i]['child']) {
                                    $catids = array_merge($catids, $module['category'][$i]['catids']);
                                } else {
                                    $catids = array_merge($catids, array($i));
                                }
                            }
                            if ($catids) {
                                $cat_where = '`catid` IN ('.implode(',', $catids).')';
                            }
                            unset($catids);
                        }
                        unset($temp);
                    ...
                    if ($cat_where) {
                        $where = $cat_where.' AND ('.implode(' OR ', $where).')';
                    }
                } else {
            ...
            case 'comment': // 评论查询
                ...
                if (!$cache && is_array($data)) {
                    if (1 == $system['more']) {
                        foreach ($data as $i => $t) {
                            if ($t['in_reply']) {
                                $data[$i]['replys'] = $this->_query("SELECT ".($system['field'] ? $system['field'] : "*")." FROM $sql_from WHERE `reply`=".$t['id']." ORDER BY `inputtime` DESC", $system['site'], $system['cache']);
                            } else {
                                $data[$i]['replys'] = array();
                            }
                        }
            ...
            case 'module': // 模块数据
                ...
                if (!$system['order']
                    ...
                    if (!$system['order']) {
                        if ($system['flag']) {
                            $system['order'] = 'updatetime_desc'; // 默认排序参数
                        } else {
                            $system['order'] = $action == 'hits' ? 'hits' : 'updatetime'; // 默认排序参数
                        }
                    }
                }
                ...
                // 商品有效期
                if (isset($fields['order_etime'])) {
                    if ($system['oot']) {
                        // 过期的
                        $sql_where.= ' AND `order_etime` BETWEEN 1 AND '.SYS_TIME;
                    } else {
                        // 正常的
                        $sql_where.= ' AND NOT (`order_etime` BETWEEN 1 AND '.SYS_TIME.')';
                    }
                }
    ...
    private function _get_select_field($field) {
        ...
        if ($this->pos_order) {
            if ($this->ci->my_position) {
                $field.= ',ROUND(6378.138*2*ASIN(SQRT(POW(SIN(('.$this->ci->my_position['lat'].'*PI()/180-'.$this->pos_order.'_lat*PI()/180)/2),2)+COS('.$this->ci->my_position['lat'].'*PI()/180)*COS('.$this->pos_order.'_lat*PI()/180)*POW(SIN(('.$this->ci->my_position['lng'].'*PI()/180-'.$this->pos_order.'_lng*PI()/180)/2),2)))*1000) AS '.$this->pos_order;
            } else {
                $this->ci->msg('没有定位到您的坐标');
            }
        }
