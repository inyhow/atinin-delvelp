...
class Navigator extends M_Controller {
    ...
    public function index() {
		...
		if ($data) {
			foreach($data as $t) {
				...
				if ($this->is_auth('admin/navigator/edit')) {
					$t['option'].= '<a class="aedit" title="'.fc_lang('修改').'" href="'.$edit.'"> <i class="fa fa-edit"></i> '.fc_lang('修改').'</a>';
				}
		...
		if ($this->is_auth('admin/navigator/edit')) {
			$str.= "<a href='".dr_url('navigator/target')."&id=\$id'><img src='".THEME_PATH."admin/images/\$target.gif' /></a>";
		} else {
			$str.= "<img src='".THEME_PATH."admin/images/\$target.gif' />";
		}
		$str.= "</td>";
		$str.= "<td style='text-align:center'>";
		if ($this->is_auth('admin/navigator/edit')) {
			$str.= "<a href='".dr_url('navigator/show')."&id=\$id'><img src='".THEME_PATH."admin/images/\$show.gif' /></a>";
		} else {
			$str.= "<img src='".THEME_PATH."admin/images/\$show.gif' />";
		}
		$str.= "</td>";
