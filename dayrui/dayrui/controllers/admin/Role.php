...
class Role extends M_Controller {
    ...
    public function index() {
		...
		if (IS_POST) {
            ...
            foreach ($ids as $i) {
                if (!$this->auth_model->role_level($this->memebr['adminid'], $i)) {
	...
	public function auth() {
        ...
        if ($MOD) {
            foreach ($MOD as $m) {
                ...
                if (!$this->site[SITE_ID]->query("SHOW TABLES LIKE '".$table."'")->row_array()) {
                    continue;
                }
                ...
                $category = $this->site[SITE_ID]->get($table)->result_array();
