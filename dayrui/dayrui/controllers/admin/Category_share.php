...
class Category_share extends M_Controller {
    ...
    public function delete($ids) {
        ...
        $data = $this->category_share_model->link->select('tableid,mid,id')->where_in('catid', $catid)->get(SITE_ID.'_share')->result_array();
        if ($data) {
            // 逐一删除内容
            foreach ($data as $t) {
        ...
        // 删除栏目
        $this->category_share_model->link->where_in('id', $catid)->delete($this->category_share_model->tablename);

        $this->load->model('attachment_model');
        foreach ($catid as $id) {
            // 删除导航数据
            $this->category_share_model->link->where('mark', 'module-'.APP_DIR.'-'.$id)->delete(SITE_ID.'_navigator');
            // 删除栏目附件
    ...
    public function html() {
        if ($this->is_auth('admin/category_share/edit')) {
            ...
            $this->category_share_model->link->where('id', $id)->update(
            ...
            if ($catid) {
                foreach ($catid as $id) {
                    ...
                    if ($setting) {
                        ...
                        $this->category_share_model->link->where('id', $id)->update($this->category_share_model->tablename, array(
    ...
    public function index() {
        ...
        if (IS_POST) {
            ...
            if ($this->input->post('action') == 'order') {
                ...
                foreach ($ids as $id) {
                    $this->category_share_model->link->where('id', $id)->update($this->category_share_model->tablename, $data[$id]);
                }
                $this->clear_cache('module');
    ...
    public function rule() {
        ...
        if (IS_POST) {
            ...
            $this->category_share_model->link->where('id', $catid)->update($this->category_share_model->tablename, array(
