...
class Check extends M_Controller {
    ...
    private function _zend() {
        ...
        if (strpos($html, 'zend optimizer') !== FALSE || strpos($html, 'zend guard') !== FALSE) {
			$zend = 1;
		}
		if (!$zend) {
			return $this->halt("您的系统环境不支持Zend Optimizer或者Zend Guard，启用之后会大大提高程序的运行效率。", 0);
		}
	}
    ...
    private function _always_populate_raw_post_data() {
        ...
        if (version_compare(PHP_VERSION, '5.6') >= 0 && ini_get('always_populate_raw_post_data') == 0) {
            return $this->halt("请务必在php.ini中把always_populate_raw_post_data设置为-1，否则系统部分功能将无法使用。", 0);
        }
    ...
    private function _upload() {
        ...
        $str = '';
        if ($file >= $post) {
            $str.= $this->halt('系统配置不合理，post_max_size值必须大于upload_max_filesize值，否则会出现“进度条100%卡住”或者提示“游客不允许上传”，解决方案：<a style="color:red" target="_blank" href="'.SYS_HELP_URL.'2061">这里</a>', 0);
        }
        if ($file < 10) {
            $str.= $this->halt('系统环境只允许上传'.$file.'MB文件，可以设置upload_max_filesize值提升上传大小，解决方案：<a style="color:red" target="_blank" href="'.SYS_HELP_URL.'2061">这里</a>', 1);
        }
        if ($post < 10) {
            $str.= $this->halt('系统环境要求每次发布内容不能超过'.$post.'MB（含文件），可以设置post_max_size值提升发布大小，解决方案：<a style="color:red" target="_blank" href="'.SYS_HELP_URL.'2061">这里</a>', 1);
        }
    ...
    private function _dir_write() {
		...
		$dir = array(
            ...
            WEBPATH.'cache/data/' => '无法生成缓存文件',
            WEBPATH.'cache/template/' => '无法生成模板解析文件',
            WEBPATH.'cache/thumb/' => '无法生成缩略图缓存文件',
        ...
        if ($local) {
            foreach ($local as $dir) {
                if (is_file(FCPATH.'module/'.$dir.'/config/module.php')) {
                    $dir[FCPATH.'module/'.$dir.'/config/'] = '无法生成模块【'.$dir.'】配置文件';
                }
            }
        }

        $local = dr_dir_map(FCPATH.'module/member/pay/', 1); // 支付接口
        if ($local) {
            foreach ($local as $dir) {
                if (is_file(FCPATH.'module/member/pay/'.$dir.'/config.php')) {
                    $dir[FCPATH.'module/member/pay/'.$dir.'/config.php'] = '无法生成支付接口【'.$dir.'】配置文件';
                }
            }
        }
