...
class Block extends M_Controller {
    ...
    public function __construct() {
		...
		$this->template->assign(array(
			'menu' => $this->get_menu_v3(array(
				fc_lang('资料块') => array('admin/block/index', 'file'),
    ...
    public function index() {
		...
		if (IS_POST) {
            ...
            $this->system_log('删除资料块【#'.@implode(',', $ids).'】'); // 记录日志
            $this->cache(1);
    ...
    public function add() {
		...
		if (IS_POST) {
			...
			$this->cache(1);
            $this->system_log('添加资料块【'.$data['name'].'#'.$id.'】'); // 记录日志
    ...
    public function edit() {
		...
		if (IS_POST) {
			...
			$this->link->where('id',(int)$id)->update($this->tablename, $data);
			$this->attachment_handle($this->uid, $this->tablename.'-'.$id, $this->field, $post);
			$this->cache(1);
            $this->system_log('修改资料块【'.$data['name'].'#'.$id.'】'); // 记录日志
			$this->admin_msg(fc_lang('操作成功，正在刷新...'), dr_url('block/index'), 1);
		}
