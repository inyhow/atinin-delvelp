...
class Root extends M_Controller {
    ...
    public function add() {
		...
		if (IS_POST) {
			...
			if (!$data['adminid'] || !isset($role[$data['adminid']])) {
                exit(dr_json(0, fc_lang('请选择角色组或者角色组不存在'), 'adminid'));
            }
	...
	public function check_username() {
		$result = $this->db
                       ...
                       ->row_array();
		if (!$result) {
            exit(dr_json(1, fc_lang('没有注册，请填写下面的邮箱和密码')));
        }
        // 不存在，注册新会员
		if ($result['adminid'] > 0) {
            exit(dr_json(2, fc_lang('该会员已经是管理组了')));
        }
        // 已经属于管理组
		exit(dr_json(0, fc_lang('已经注册，可以直接添加管理组'), $result['uid'])); // 已经注册会员
	}
