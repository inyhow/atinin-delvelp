...
class Api extends M_Controller {
        ...
        if (IS_POST) {
            ...
            if ($this->upload->do_upload('Filedata')) {
                ...
                if (!is_file(WEBPATH.$file)) {
                    $file = THEME_PATH.'admin/images/ext/blank.gif';
                }
	...
	public function member() {
        ...
        if ($uid == 'guest' || !$uid) {
            exit('<div style="padding-top:50px;color:blue;font-size:14px;text-align:center">'.fc_lang('游客').'</div>');
        }
        if (is_numeric($uid)) {
	...
	public function testftp() {
		...
		if (!$this->ftp->upload(WEBPATH.'index.php', $path.'/test.ftp', $mode, 0775)) {
            exit(fc_lang('Ftp服务器无上传权限'));
        }
        ...
        if (isset($GLOBALS["HTTP_RAW_POST_DATA"])) {
            ...
            if (isset($_GET['image']) && !empty($_GET['image'])) {
                ...
                if ($id && $info = get_attachment($id)) {
                    ...
                    if (!is_dir(SYS_UPLOAD_PATH.$path)) {
                        dr_mkdirs(SYS_UPLOAD_PATH.$path);
                    }
    ...
    public function ajax_edit_image() {
        ...
        if (is_numeric($file) && $info = get_attachment($file)) {
            if (isset($info['remote']) && $info['remote']) {
                ...
                $image = SYS_ATTACHMENT_URL.$info['attachment'];
            }
            unset($info);
        } else {
            $image = $file;
