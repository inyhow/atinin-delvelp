...
class Api extends M_Controller {
	...
	public function hits() {
        ...
        if (SYS_UPDATE) {
            ...
            $this->site[SITE_ID]->query(trim(str_replace('{tablename}', $this->db->dbprefix(SITE_ID.'_'.$dir.'_hits'), $this->module_model->system_table['hits'])));
        }
        ...
        // 更新主表
		$this->site[SITE_ID]->where('id', $id)->update(SITE_ID.'_'.$dir, array('hits' => $hits));
		if ($mod['share']) {
            $this->site[SITE_ID]->where('id', $id)->update(SITE_ID.'_share', array('hits' => $hits));
        }

        // 获取统计数据
        $total = $this->site[SITE_ID]->where('id', $id)->get($this->db->dbprefix(SITE_ID.'_'.$dir.'_hits'))->row_array();
        if (!$total) {
            $total['day_hits'] = $total['week_hits'] = $total['month_hits'] = $total['year_hits'] = 1;
        }

        // 更新到统计表
        $this->site[SITE_ID]->replace($this->db->dbprefix(SITE_ID.'_'.$dir.'_hits'), array(
            'id' => $id,
            'hits' => $hits,
